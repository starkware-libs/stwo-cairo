...
use clap::Parser;
use dev_utils::utils::{
    read_cairo_arguments_from_file, run_cairo1_and_adapter, run_program_and_adapter, Error,
};
...
fn main() -> Result<(), Error> {
    ...
    let prover_input = if args.cairo1 {
        ...
        let args = if let Some(ref path) = args.program_arguments_file {
            read_cairo_arguments_from_file(path)
        } else {
            Vec::new()
        };
