...
impl MemoryBuilder {
    ...
    pub fn from_relocatble_memory(
        ...
        // In order all the memory values until that not used address will be set to default.
        builder.set(
            relocation_table[relocation_table.len() - 1],
            MemoryValue::Small(0),
        );
        builder
    }

    pub fn get_relocation_table(&self) -> &[u32] {
        self.memory
            .config
            .relocation_table
            .as_ref()
            .expect("Relocation table not set")
    }
