...
impl StateTransitions {
    ...
    pub fn from_trace_generic_mode(
        trace: Vec<RelocatedTraceEntry>,
        memory: &MemoryBuilder,
    ) -> Self {
        let _span = span!(Level::INFO, "StateTransitions::from_trace_generic_mode").entered();
...
mod mappings_tests {
    ...
    fn test_generic_mode() {
        ...
        let states = StateTransitions::from_trace_generic_mode(trace.to_vec(), &memory_builder);
    ...
    fn test_generic_mode_fail_non_stone_opcode() {
        ...
        let mut memory_builder = MemoryBuilder::new(MemoryConfig::default());
        memory_builder.set(1, MemoryValue::F252([x[0], x[1], x[2], x[3], 0, 0, 0, 0]));
        let states = StateTransitions::from_trace_generic_mode(
            [
