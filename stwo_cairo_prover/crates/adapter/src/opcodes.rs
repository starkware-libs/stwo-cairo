...
fn assert_state_in_address_space(casm_state: CasmState) {
    assert!(
        (casm_state.ap.0 as usize) < 1 << 27, // MEMORY_ADDRESS_BOUND,
        "AP out of address range: {}",
        casm_state.ap.0
    );
    assert!(
        (casm_state.fp.0 as usize) < 1 << 27, // MEMORY_ADDRESS_BOUND,
        "FP out of address range: {}",
        casm_state.fp.0
    );
    assert!(
        (casm_state.pc.0 as usize) < 1 << 27, // MEMORY_ADDRESS_BOUND,
