...
const SMALL_ADD_POSITIVE_UPPER_BOUND: u128 = 2_u128.pow(27) - 1;
const SMALL_ADD_NEGATIVE_UPPER_BOUND: [u32; 8] = P_MIN_1;
const SMALL_ADD_NEGATIVE_LOWER_BOUND: [u32; 8] = P_MIN_2_TO_27;
...
fn is_small_add(dst: MemoryValue, op0: MemoryValue, op_1: MemoryValue) -> bool {
    [dst, op0, op_1].iter().all(|val| {
        ...
        value <= U256::from(SMALL_ADD_POSITIVE_UPPER_BOUND)
            || value >= u256_from_le_array(SMALL_ADD_NEGATIVE_LOWER_BOUND)
                && value <= u256_from_le_array(SMALL_ADD_NEGATIVE_UPPER_BOUND)
    })
}
