...
impl CasmStatesByOpcode {
    ...
    pub fn merge(
        ...
        self.blake2s_opcode.extend(blake2s_opcode);
        self.qm31_add_mul_opcode.extend(qm31_add_mul_opcode);
    }
...
impl StateTransitions {
    ...
    pub fn from_iter(
        ...
        let initial_state = iter.peek().copied().expect("Empty trace").into();

        // TODO(Stav): Check if the adapter outputs the final state.
        // TODO(Stav): Check if this make sense, a trace with 1 state might not be valid.
    ...
    pub fn from_slice_parallel(trace: &[RelocatedTraceEntry], memory: &MemoryBuilder) -> Self {
        ...
        if trace.len() <= N_CHUNKS {
            return Self::from_iter(trace.iter().copied(), memory);
        };
        let initial_state = trace.first().copied().unwrap().into();
        let final_state = trace.last().copied().unwrap().into();

        let chunk_size = trace.len() / N_CHUNKS;

        // TODO(Ohad): use the global pool when stack overflow issue is resolved.
        let pool = rayon::ThreadPoolBuilder::new().build().unwrap();
        let casm_states_by_opcode = pool.install(|| {
            trace
