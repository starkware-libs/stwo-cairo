...
impl StateTransitions {
    ...
    pub fn from_slice_generic_mode(trace: &[RelocatedTraceEntry], memory: &MemoryBuilder) -> Self {
        ...
        // Assert all the instruction in the trace are provable by the generic opcode component.
        trace.par_iter().for_each(|entry| {
        let encoded_instruction = memory.get_inst(entry.pc as u32);
        let instruction = Instruction::decode(encoded_instruction);
        if !matches!(instruction.opcode_extension, OpcodeExtension::Stone) {
            panic!("generic_mode is enabled but some instruction can't be proved by the generic opcode component.");
        }
        });
        ...
        let casm_states_by_opcode = CasmStatesByOpcode {
            generic_opcode: trace.iter().map(|entry| CasmState::from(*entry)).collect(),
            ..Default::default()
        };
...
mod mappings_tests {
    ...
    #[test]
    #[should_panic(
        expected = "generic_mode is enabled but some instruction can't be proved by the generic opcode component."
    )]
    fn test_generic_mode_fail_non_stone_opcode() {
        ...
        let mut memory_builder = MemoryBuilder::new(MemoryConfig::default());
        memory_builder.set(1, MemoryValue::F252([x[0], x[1], x[2], x[3], 0, 0, 0, 0]));
        StateTransitions::from_slice_generic_mode(
