...
use crate::builtins::{
    BuiltinSegments, ADD_MOD_MEMORY_CELLS, BITWISE_MEMORY_CELLS, ECDSA_MEMORY_CELLS, EC_OP_MEMORY_CELLS, KECCAK_MEMORY_CELLS, MUL_MOD_MEMORY_CELLS, PEDERSEN_MEMORY_CELLS, POSEIDON_MEMORY_CELLS, RANGE_CHECK_MEMORY_CELLS
};
...
impl Relocator {
    ...
    // Return the segment info (start_address, exclusive stop_address) for each builtin.
    pub fn get_builtin_segments(&self) -> BuiltinSegments{
        ...
        for (segment_index, builtin_name) in self.builtins_segments_indices.iter(){
            ...
            let stop_addr = self.relocation_table[*segment_index + 1];
