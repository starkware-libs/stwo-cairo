...
impl Relocator {
    ...
    // Each segment size must be greater than 16.
    pub fn new(
    ...
    // For builtins segments we want to pad with real values.
    ...
    fn get_relocatable_segment(&self, segment_index: usize) -> Vec<(u32, [u32; 8])> {
        let mut res = vec![];
    ...
    pub fn get_relocatable_memory(&self) -> Vec<(u32, [u32; 8])> {
        let mut res = vec![];
