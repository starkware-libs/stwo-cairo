...
mod tests {
    ...
    fn test_add_small() {
        let instructions = casm! {
            ...
            [ap] = 134217725, ap++;
            [ap] = 2, ap++;
            // The sum is 2^27-1 which is the maximal value for a small number.
            [ap] = [fp] + [ap-1], ap++;
            // The sum is 2^27-1 which is the maximal value for a small number.
    ...
    fn test_add_big() {
        let instructions = casm! {
            ...
            [ap] = 134217725, ap++;
            [ap] = 3, ap++;
            // The sum is 2^27 which is the minimal value for a big number.
            [ap] = [fp] + [ap-1], ap++;
            [ap] = [ap-1] + 1, ap++;
            [ap] = 1, ap++;
        }
    ...
    fn test_mul_small() {
        let instructions = casm! {
            [ap] =  7, ap++;
            [ap] = 19173961, ap++;
            // The multiplication result is 2^27 - 1 which is the maximal value for a small number.
            [ap] = [ap-1] * [ap-2], ap++;
            [ap] = [ap-1]*7, ap++;
            [ap] = 1, ap++;
        }
