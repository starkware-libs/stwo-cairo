...
use stwo_cairo_common::builtins_constants::*;
    ...
    pub fn fill_components(
        ...
        self.qm_31_add_mul_opcode = components.contains(&"qm_31_add_mul_opcode").then(|| {
            qm_31_add_mul_opcode::ClaimGenerator::new(
                casm_states_by_opcode.qm_31_add_mul_opcode.clone(),
            )
        });

        self.ret_opcode = components
            .contains(&"ret_opcode")
            .then(|| ret_opcode::ClaimGenerator::new(casm_states_by_opcode.ret_opcode.clone()));

        self.mul_opcode = components
            .contains(&"mul_opcode")
            .then(|| mul_opcode::ClaimGenerator::new(casm_states_by_opcode.mul_opcode.clone()));

        self.mul_opcode_small = components.contains(&"mul_opcode_small").then(|| {
            mul_opcode_small::ClaimGenerator::new(casm_states_by_opcode.mul_opcode_small.clone())
        });

        self.jump_opcode_abs = components.contains(&"jump_opcode_abs").then(|| {
            jump_opcode_abs::ClaimGenerator::new(casm_states_by_opcode.jump_opcode_abs.clone())
        });
        ...
        self.jump_opcode_double_deref =
            components.contains(&"jump_opcode_double_deref").then(|| {
                jump_opcode_double_deref::ClaimGenerator::new(
                    casm_states_by_opcode.jump_opcode_double_deref.clone(),
                )
            });

        self.jump_opcode_rel = components.contains(&"jump_opcode_rel").then(|| {
            jump_opcode_rel::ClaimGenerator::new(casm_states_by_opcode.jump_opcode_rel.clone())
        });

        self.jump_opcode_rel_imm = components.contains(&"jump_opcode_rel_imm").then(|| {
            jump_opcode_rel_imm::ClaimGenerator::new(
                casm_states_by_opcode.jump_opcode_rel_imm.clone(),
            )
        });

        self.jnz_opcode_non_taken = components.contains(&"jnz_opcode_non_taken").then(|| {
            jnz_opcode_non_taken::ClaimGenerator::new(
                casm_states_by_opcode.jnz_opcode_non_taken.clone(),
            )
        });

        self.jnz_opcode_taken = components.contains(&"jnz_opcode_taken").then(|| {
            jnz_opcode_taken::ClaimGenerator::new(casm_states_by_opcode.jnz_opcode_taken.clone())
        });

        self.call_opcode_rel_imm = components.contains(&"call_opcode_rel_imm").then(|| {
            call_opcode_rel_imm::ClaimGenerator::new(
                casm_states_by_opcode.call_opcode_rel_imm.clone(),
            )
        });

        self.call_opcode_abs = components.contains(&"call_opcode_abs").then(|| {
            call_opcode_abs::ClaimGenerator::new(casm_states_by_opcode.call_opcode_abs.clone())
        });

        self.assert_eq_opcode_imm = components.contains(&"assert_eq_opcode_imm").then(|| {
            assert_eq_opcode_imm::ClaimGenerator::new(
                casm_states_by_opcode.assert_eq_opcode_imm.clone(),
            )
        });
        ...
        self.assert_eq_opcode_double_deref = components
            ...
            .then(|| {
                assert_eq_opcode_double_deref::ClaimGenerator::new(
                    casm_states_by_opcode.assert_eq_opcode_double_deref.clone(),
                )
            });

        self.assert_eq_opcode = components.contains(&"assert_eq_opcode").then(|| {
            assert_eq_opcode::ClaimGenerator::new(casm_states_by_opcode.assert_eq_opcode.clone())
        });

        self.add_opcode = components
            .contains(&"add_opcode")
            .then(|| add_opcode::ClaimGenerator::new(casm_states_by_opcode.add_opcode.clone()));

        self.add_opcode_small = components.contains(&"add_opcode_small").then(|| {
            add_opcode_small::ClaimGenerator::new(casm_states_by_opcode.add_opcode_small.clone())
        });

        self.add_ap_opcode = components.contains(&"add_ap_opcode").then(|| {
            add_ap_opcode::ClaimGenerator::new(casm_states_by_opcode.add_ap_opcode.clone())
        });

        self.generic_opcode = components.contains(&"generic_opcode").then(|| {
            generic_opcode::ClaimGenerator::new(casm_states_by_opcode.generic_opcode.clone())
        });
        ...
        self.pedersen_points_table_window_bits_18 = components
            ...
            .then(|| {
                pedersen_points_table_window_bits_18::ClaimGenerator::new(Arc::clone(
                    preprocessed_trace,
                ))
            });
