...
impl OpcodesInteractionClaimGenerator {
    pub fn write_interaction_trace(
        ...
        rayon::scope(|s| {
            ...
            s.spawn(|_| {
                *add_small_result.lock().unwrap() =
                    Some(process_interaction_gens(self.add_small, common_lookup_elements));
            });
            s.spawn(|_| {
                *add_ap_result.lock().unwrap() =
                    Some(process_interaction_gens(self.add_ap, common_lookup_elements));
            });
            s.spawn(|_| {
                *assert_eq_result.lock().unwrap() =
                    Some(process_interaction_gens(self.assert_eq, common_lookup_elements));
            });
            s.spawn(|_| {
            ...
            s.spawn(|_| {
                *jnz_taken_result.lock().unwrap() =
                    Some(process_interaction_gens(self.jnz_taken, common_lookup_elements));
            });
            s.spawn(|_| {
            ...
            s.spawn(|_| {
                *jump_rel_result.lock().unwrap() =
                    Some(process_interaction_gens(self.jump_rel, common_lookup_elements));
            });
            s.spawn(|_| {
            ...
            s.spawn(|_| {
                *mul_small_result.lock().unwrap() =
                    Some(process_interaction_gens(self.mul_small, common_lookup_elements));
            });
            s.spawn(|_| {
        ...
        // Sequentially extend the tree builder with all collected evaluations in deterministic order
