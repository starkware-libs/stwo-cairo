...
impl CairoClaimGenerator {
    ...
    pub fn write_trace(
        ...
        let (blake_round_claim, blake_round_interaction_gen) = self
            .blake_round_trace_generator
            .map(|gen| {
                let mut blake_g_trace_generator = self.blake_g_trace_generator.take().unwrap();
                let blake_sigma_trace_generator = self.blake_sigma_trace_generator.take().unwrap();
                ...
                self.blake_g_trace_generator = Some(blake_g_trace_generator);
                self.blake_sigma_trace_generator = Some(blake_sigma_trace_generator);
        ...
        let (blake_g_claim, blake_g_interaction_gen) = self
            .blake_g_trace_generator
            .map(|gen| {
                let verify_bitwise_xor_12_trace_generator =
                    self.verify_bitwise_xor_12_trace_generator.take().unwrap();

                let result = gen.write_trace(
                    tree_builder,
                    &self.verify_bitwise_xor_8_trace_generator,
                ...
                self.verify_bitwise_xor_12_trace_generator =
                    Some(verify_bitwise_xor_12_trace_generator);
