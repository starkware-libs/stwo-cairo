...
use rayon::{join, scope, ThreadPoolBuilder};
...
impl CairoInteractionClaimGenerator {
    pub fn write_interaction_trace(
        ...
        // Create a dedicated thread pool for partial_ec_mul (typically the longest task).
        let partial_ec_mul_pool = ThreadPoolBuilder::new().num_threads(16).build().unwrap();

        join(
            || {
                // Pedersen partial_ec_mul in dedicated 16-thread pool
                partial_ec_mul_result = Some(partial_ec_mul_pool.install(|| {
                    partial_ec_mul_gen
