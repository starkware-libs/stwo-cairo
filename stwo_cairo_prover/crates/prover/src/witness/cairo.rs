...
use crate::witness::builtins::builtins_write_trace;
...
use crate::witness::opcodes::opcodes_write_trace;
use crate::witness::prelude::{PreProcessedTrace, M31};
use crate::witness::range_checks::range_checks_write_trace;
...
pub fn create_cairo_claim_generator(
    ...
    let top_level_components =
        get_top_level_components(&state_transitions.casm_states_by_opcode, &builtin_segments);
    let all_components = get_all_components(&top_level_components);
    ...
    let memory_address_to_id = cairo_claim_generator.memory_address_to_id.as_ref().unwrap();
    let memory_id_to_big = cairo_claim_generator.memory_id_to_big.as_ref().unwrap();
    // Yield public memory.
    ...
    {
        let id = memory_address_to_id.get_id(addr);
        memory_address_to_id.add_input(&addr);
    ...
    let final_ap = state_transitions.final_state.ap.0;
    ...
    let public_data = PublicData {
        ...
        final_state: state_transitions.final_state,
    };
...
impl CairoClaimGenerator {
    pub fn write_trace(
        ...
        let (builtins_claim, builtins_interaction_gen) = builtins_write_trace(
            ...
            self.memory_address_to_id.as_ref(),
            self.memory_id_to_big.as_ref(),
            self.pedersen_aggregator.as_ref(),
...
fn get_top_level_components(
    casm_states_by_opcodes: &CasmStatesByOpcode,
    builtin_segments: &BuiltinSegments,
) -> Vec<&'static str> {
    let mut top_level_components = vec![];
    if !casm_states_by_opcodes.generic_opcode.is_empty() {
        top_level_components.push("generic_opcode");
    }
    if !casm_states_by_opcodes.add_ap_opcode.is_empty() {
        top_level_components.push("add_ap_opcode");
    }
    if !casm_states_by_opcodes.add_opcode.is_empty() {
        top_level_components.push("add_opcode");
    }
    if !casm_states_by_opcodes.add_opcode_small.is_empty() {
        top_level_components.push("add_opcode_small");
    }
    if !casm_states_by_opcodes.assert_eq_opcode.is_empty() {
        top_level_components.push("assert_eq_opcode");
    }
    if !casm_states_by_opcodes
        .assert_eq_opcode_double_deref
        .is_empty()
    {
        top_level_components.push("assert_eq_opcode_double_deref");
    }
    if !casm_states_by_opcodes.assert_eq_opcode_imm.is_empty() {
        top_level_components.push("assert_eq_opcode_imm");
    }
    if !casm_states_by_opcodes.call_opcode_abs.is_empty() {
        top_level_components.push("call_opcode_abs");
    }
    if !casm_states_by_opcodes.call_opcode_rel_imm.is_empty() {
        top_level_components.push("call_opcode_rel_imm");
    }
    if !casm_states_by_opcodes.jnz_opcode_non_taken.is_empty() {
        top_level_components.push("jnz_opcode_non_taken");
    }
    if !casm_states_by_opcodes.jnz_opcode_taken.is_empty() {
        top_level_components.push("jnz_opcode_taken");
    }
    if !casm_states_by_opcodes.jump_opcode_rel_imm.is_empty() {
        top_level_components.push("jump_opcode_rel_imm");
    }
    if !casm_states_by_opcodes.jump_opcode_rel.is_empty() {
        top_level_components.push("jump_opcode_rel");
    }
    if !casm_states_by_opcodes.jump_opcode_double_deref.is_empty() {
        top_level_components.push("jump_opcode_double_deref");
    }
    if !casm_states_by_opcodes.jump_opcode_abs.is_empty() {
        top_level_components.push("jump_opcode_abs");
    }
    if !casm_states_by_opcodes.mul_opcode_small.is_empty() {
        top_level_components.push("mul_opcode_small");
    }
    if !casm_states_by_opcodes.mul_opcode.is_empty() {
        top_level_components.push("mul_opcode");
    }
    if !casm_states_by_opcodes.ret_opcode.is_empty() {
        top_level_components.push("ret_opcode");
    }
    if !casm_states_by_opcodes.blake_compress_opcode.is_empty() {
        top_level_components.push("blake_compress_opcode");
    }
    if !casm_states_by_opcodes.qm_31_add_mul_opcode.is_empty() {
        top_level_components.push("qm_31_add_mul_opcode");
    }
    if builtin_segments.pedersen_builtin.is_some() {
        top_level_components.push("pedersen_builtin");
    }
    if builtin_segments.poseidon_builtin.is_some() {
        top_level_components.push("poseidon_builtin");
    }
    if builtin_segments.range_check96_builtin.is_some() {
        top_level_components.push("range_check96_builtin");
    }
    if builtin_segments.range_check_builtin.is_some() {
        top_level_components.push("range_check_builtin");
    }
    if builtin_segments.bitwise_builtin.is_some() {
        top_level_components.push("bitwise_builtin");
    }
    if builtin_segments.add_mod_builtin.is_some() {
        top_level_components.push("add_mod_builtin");
    }
    if builtin_segments.mul_mod_builtin.is_some() {
        top_level_components.push("mul_mod_builtin");
    }
    top_level_components
}

fn get_all_components(top_level_components: &[&str]) -> IndexSet<&'static str> {
    let mut components = IndexSet::new();
    for component in top_level_components {
        components.extend(get_sub_components(component));
    }
    components
}
