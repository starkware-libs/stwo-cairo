...
impl InteractionClaimGenerator {
    pub fn write_interaction_trace(
        ...
        // Compute denominators for columns 0 and 1 in parallel.
        let (denoms_0, denoms_1) = rayon::join(
            || {
                (0..n_rows)
                    .map(|i| {
                        let d0: PackedQM31 =
                            common_lookup_elements.combine(&self.lookup_data.verify_instruction_0[i]);
                        let d1: PackedQM31 =
                            common_lookup_elements.combine(&self.lookup_data.memory_address_to_id_0[i]);
                        (d0 + d1, d0 * d1)
                    })
                    .collect::<Vec<_>>()
            },
            || {
                (0..n_rows)
                    .map(|i| {
                        let d0: PackedQM31 =
                            common_lookup_elements.combine(&self.lookup_data.memory_id_to_big_0[i]);
                        let d1: PackedQM31 =
                            common_lookup_elements.combine(&self.lookup_data.memory_address_to_id_1[i]);
                        (d0 + d1, d0 * d1)
                    })
                    .collect::<Vec<_>>()
            },
        ...
        // Compute denominators for columns 2 and 3 in parallel.
        let (denoms_2, denoms_3) = rayon::join(
            || {
                (0..n_rows)
                    .map(|i| {
                        let d0: PackedQM31 =
                            common_lookup_elements.combine(&self.lookup_data.memory_address_to_id_2[i]);
                        let d1: PackedQM31 =
