...
impl PartialEcMul {
    pub fn deduce_output(
        ...
        assert!((round.0 as usize) < NUM_WINDOWS);
        ...
        let window_value = m_shifted[0];
        let table_row =
            table_offset + round * M31::from_u32_unchecked(ROWS_PER_WINDOW as u32) + window_value;
        let table_point: ProjectivePoint = PEDERSEN_TABLE
            .get_row(table_row.0 as usize)
            .try_into()
