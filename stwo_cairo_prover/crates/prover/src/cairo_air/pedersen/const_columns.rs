...
use stwo_cairo_common::preprocessed_consts::pedersen::{
    BITS_PER_WINDOW, NUM_WINDOWS, PEDERSEN_TABLE_N_ROWS,
};
...
static PEDERSEN_TABLE: LazyLock<PedersenPointsTable> = LazyLock::new(PedersenPointsTable::new);
...
struct PedersenPointsTable {
    // The one copy of the table data. Shared by all column instances.
...
impl PedersenPointsTable {
    fn new() -> Self {
        Self {
            column_data: Self::gen_columns_data(),
        }
    }
    ...
    fn f252_to_limbs(x: Felt) -> [BaseField; FELT252_N_WORDS] {
        let x_bytes = x.to_bytes_le();
        const MASK: u32 = (1 << FELT252_BITS_PER_WORD) - 1;

        from_fn(|limb| {
            let first_bit = limb * FELT252_BITS_PER_WORD;
            let last_bit = first_bit + FELT252_BITS_PER_WORD - 1;
            let first_byte = first_bit / 8;
            let last_byte = last_bit / 8;
            assert!(last_byte == first_byte + 1); // Each limb spans exactly two bytes
            let word = (x_bytes[first_byte] as u32) + ((x_bytes[last_byte] as u32) << 8);
            BaseField::from_u32_unchecked((word >> (first_bit % 8)) & MASK)
        })
    }
    ...
    fn create_block(
        ...
        n_rows: usize,
    ) -> Vec<[BaseField; PEDERSEN_TABLE_N_COLUMNS]> {
        ...
        let z_inverses = Self::felt_batch_inverse(&block_points_zs);

        // Compute the affine x,y for each point and serialize a row
        for ((x, y), z_inv) in block_points_xs
            .iter()
            .zip(block_points_ys.iter())
            .zip(z_inverses.iter())
        {
            let x_limbs = Self::f252_to_limbs(x * z_inv);
            let y_limbs = Self::f252_to_limbs(y * z_inv);
            result.push(
                x_limbs
                    .into_iter()
                    .chain(y_limbs.into_iter())
                    .collect::<Vec<_>>()
                    .try_into()
                    .expect("should have the correct number of columns"),
            );
        }
        result
    }

    fn add_p0_or_p2_section(
        rows: &mut Vec<[BaseField; PEDERSEN_TABLE_N_COLUMNS]>,
        point: &ProjectivePoint,
    ) {
        ...
        for _ in 0..NUM_WINDOWS {
            ...
            threads.push(spawn(move || {
                Self::create_block(&point_for_thread, 1 << BITS_PER_WINDOW)
            }));
        ...
        for t in threads {
            let mut block = t.join().expect("Thread succeeded");
            rows.append(&mut block);
        }
    }

    fn gen_columns_data() -> [Vec<BaseField>; PEDERSEN_TABLE_N_COLUMNS] {
        let mut rows = vec![];
        Self::add_p0_or_p2_section(
            &mut rows,
            &ProjectivePoint::new(PEDERSEN_P0.x(), PEDERSEN_P0.y(), Felt::ONE),
        );
        rows.append(&mut Self::create_block(
            &ProjectivePoint::new(PEDERSEN_P1.x(), PEDERSEN_P1.y(), Felt::ONE),
            16,
        ));
        Self::add_p0_or_p2_section(
            &mut rows,
            &ProjectivePoint::new(PEDERSEN_P2.x(), PEDERSEN_P2.y(), Felt::ONE),
        );
        rows.append(&mut Self::create_block(
            &ProjectivePoint::new(PEDERSEN_P3.x(), PEDERSEN_P3.y(), Felt::ONE),
            16,
        ));
