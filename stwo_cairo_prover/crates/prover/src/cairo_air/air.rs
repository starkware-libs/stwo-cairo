...
use stwo_cairo_adapter::vm_import::{
    MemoryEntry, MemorySmallValue, PubMemoryEntry, PubMemoryValue, PublicSegmentRanges, SegmentRange
};
...
// (Address, Id, Value)
pub type PublicMemoryA = Vec<(u32, u32, [u32; 8])>;
...
pub struct PublicData {
    pub public_memory_a: PublicMemoryA,
...
impl PublicData {
    ...
    pub fn logup_sum(&self, lookup_elements: &CairoInteractionElements) -> QM31 {
        ...
        println!("initial_state: {:?}", self.initial_state);
        println!("final_state: {:?}", self.final_state);
        ...
        self.public_memory
            ...
            .for_each(|(addr, id, val)| {
                println!("addr: {}, id: {}, val: {:?}", addr, id, val);
...
fn extract_public_segments(
    ...
    let mut ranges = start_ptrs
        .zip(end_ptrs)
        .map(|(start_ptr, stop_ptr)| SegmentRange { start_ptr, stop_ptr });
   
    let output = builtin_segments.output.map(|_| ranges.next().unwrap());
    let range_check_128 = builtin_segments.range_check_bits_128.map(|_| ranges.next().unwrap());
    let bitwise = builtin_segments.bitwise.map(|_| ranges.next().unwrap());
    let range_check_96 = builtin_segments.range_check_bits_96.map(|_| ranges.next().unwrap());
    let add_mod = builtin_segments.add_mod.map(|_| ranges.next().unwrap());
    println!("segments, output, range_check_128, bitwise, range_check_96, add_mod: {:?}, {:?}, {:?}, {:?}, {:?}, {:?}", builtin_segments, output, range_check_128, bitwise, range_check_96, add_mod);
    PublicSegmentRanges {
        output,
        range_check_128,
        bitwise,
...
fn extract_sections_from_memory(
    ...
    // println!("program, safe_call, output: {:?}, {:?}, {:?}", program, safe_call, output);
    PublicMemory {
        program,
        safe_call,
        public_segments,
...
impl CairoClaimGenerator {
    pub fn new(input: ProverInput) -> Self {
        ...
        let public_memory =
            extract_sections_from_memory(&input.memory, input.builtins_segments, initial_pc, initial_ap, final_ap);
        // let output = output_memory_addresses
        //     .
        // Public data.
        let public_memory_a = input
            .public_memory_addresses
            .iter()
            .copied()
            .map(|addr| {
            ...
            .collect_vec();
        // let (program, output) = extract_memory_and_output(&input.memory,initial_state.ap.0,
        // initial_state.pc.0, final_state.ap.0); let public_memory_a = PublicMemory::
        let public_data = PublicData {
            public_memory_a: public_memory_a,
