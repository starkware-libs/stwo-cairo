...
mod tests {
    ...
    // Create `N_LANES` instances of the same value, except for index 4, which has a different
    // value. Used to test two different instances in a single SIMD vector.
    fn create_input<T: Copy>(common: T, special: T) -> [T; N_LANES] {
        let mut input = [common; N_LANES];
        input[4] = special;
        input
    }

    // Pack the columns in the given table to create an array of `PackedUInt32` values.
    fn pack_u32_cols_in_table<const N: usize>(input: [[u32; N]; N_LANES]) -> [PackedUInt32; N] {
        from_fn(|i| PackedUInt32::from_array(from_fn(|j| UInt32::from(input[j][i]))))
    }

    // Pack the columns in the given table to create an array of `PackedM31` values.
    fn pack_m31_cols_in_table<const N: usize>(input: [[u32; N]; N_LANES]) -> [PackedM31; N] {
        from_fn(|i| PackedM31::from_array(from_fn(|j| M31::from(input[j][i]))))
    }

    #[test]
    fn test_g() {
        let input = pack_u32_cols_in_table(create_input(
            [
        ...
        let expected = pack_u32_cols_in_table(create_input(
            [2827666065, 4146123195, 3407348176, 3638212488],
            [2049993894, 223224271, 100412452, 1063654435],
        ));
    ...
    #[test]
    fn test_round_sigma() {
        let input = PackedM31::from_array(create_input(M31::from(7), M31::from(2)));

        let expected = pack_m31_cols_in_table(create_input(
    ...
    #[test]
    fn test_blake_round() {
        let state = pack_u32_cols_in_table(create_input(
            [
        ...
        let message_pointer =
            PackedM31::from_array(create_input(M31::from(7687346), M31::from(8676)));
        let message = create_input(
            [
        ...
        let rounds = PackedM31::from_array(create_input(M31::from(5), M31::from(9)));
        let chains = PackedM31::from_array(create_input(M31::from(0), M31::from(100)));
        ...
        let memory_entries: Vec<MemoryEntry> = (0..=15)
            .flat_map(|i| {
                [
                    ...
                    MemoryEntry {
                        address: message_pointer.to_array()[4].0 as u64 + i as u64,
                        value: [message[4][i as usize], 0, 0, 0, 0, 0, 0, 0],
                    },
                ]
            })
            .collect();
