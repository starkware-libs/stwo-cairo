...
impl BlakeRound<'_> {
    fn deduce_output(
        ...
        chain: M31,
        round: M31,
        (state, mp): ([UInt32; 16], M31),
    ) -> (M31, M31, ([UInt32; 16], M31)) {
        let sigma = sigma(round.0 as usize);
        let mut m = vec![];
        for i in sigma.iter() {
            let id = self.memory_address_to_id_state.get_id(mp + *i);
            m.push(self.memory_id_to_big_state.get_small_value(&id) as u32);
        }
        ...
        (chain, round + M31::from(1), (state.map(UInt32::from), mp))
    }
}
#[cfg(test)]
mod tests {
