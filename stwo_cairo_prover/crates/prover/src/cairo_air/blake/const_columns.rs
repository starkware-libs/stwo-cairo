use stwo_cairo_common::preprocessed_consts::blake::{sigma, N_BLAKE_ROUNDS, N_BLAKE_SIGMA_COLS};
...
use crate::stwo_prover::core::backend::simd::m31::{PackedM31, N_LANES};
...
impl BlakeSigmaColumn {
    pub fn new(col: usize) -> Self {
        assert!(
            (2_u32.pow(LOG_N_ROWS)) as usize / N_LANES == 1,
            "Num packed rows assumed to be 1"
        );
        let sigma = BaseColumn::from_iter(pad(sigma_m31, N_BLAKE_ROUNDS, col)).data;
        assert!(sigma.len() == 1, "Num packed rows assumed to be 1");
        Self {
            values: sigma[0],
            col,
        }
    ...
    pub fn packed_at(&self, vec_row: usize) -> PackedM31 {
        assert!(vec_row == 0, "Num packed rows assumed to be 1");
        self.values
    }
}
