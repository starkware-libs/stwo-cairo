...
impl BuiltinsClaimGenerator {
    pub fn new(builtin_segments: BuiltinSegments) -> Self {
        ...
        let mul_mod_builtin_trace_generator = builtin_segments.mul_mod.map(|mul_mod| {
            let mul_mod_builtin_cells_per_instance =
                BuiltinSegments::builtin_memory_cells_per_instance(BuiltinName::mul_mod);
            let mul_mod_builtin_segment_length = mul_mod.stop_ptr - mul_mod.begin_addr;
            assert!(
                (mul_mod_builtin_segment_length % mul_mod_builtin_cells_per_instance) == 0,
                "mul_mod segment length is not a multiple of mul_mod_builtin_cells_per_instance"
            );
            assert!(
                (mul_mod_builtin_segment_length / mul_mod_builtin_cells_per_instance)
                    .is_power_of_two(),
                "mul_mod instances number is not a power of two"
            );
            mul_mod_builtin::ClaimGenerator::new(
                (mul_mod_builtin_segment_length / mul_mod_builtin_cells_per_instance).ilog2(),
                mul_mod.begin_addr as u32,
            )
        });
