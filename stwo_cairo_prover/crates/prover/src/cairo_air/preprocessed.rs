...
impl PreProcessedTrace {
    ...
    pub fn new() -> Self {
        let seq_columns = (LOG_N_LANES..=LOG_MAX_ROWS).map(Seq::new);
        let bitwise_xor_columns =
            (0..3).map(move |col_index| BitwiseXor::new(XOR_N_BITS, col_index));
        Self {
            seq_columns: seq_columns.into_iter().collect_vec(),
            bitwise_xor_columns: bitwise_xor_columns.collect_vec(),
        }
    }
