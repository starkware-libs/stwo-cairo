...
/// Columns for BitwiseXor preprocessed columns.
#[derive(Debug)]
enum XorColumn {
    A,
    B,
...
impl BitwiseXor {
    pub const fn new(n_bits: u32, col: usize) -> Self {
        let col = match col {
            0 => XorColumn::A,
            1 => XorColumn::B,
            2 => XorColumn::C,
            _ => panic!("col_index must be in range 0..=2"),
        };
        Self { n_bits, col }
    }
    ...
    pub fn id(&self) -> PreProcessedColumnId {
        PreProcessedColumnId {
            id: format!("bitwise_xor_{}_{:?}", self.n_bits, self.col),
        }
    }
