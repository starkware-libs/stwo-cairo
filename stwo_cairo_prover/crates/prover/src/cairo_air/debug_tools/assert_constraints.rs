...
fn assert_every_cairo_component(
    trace_polys: TreeVec<Vec<&CirclePoly<SimdBackend>>>,
    ...
    macro_rules! assert_vec {
        ($name:ident) => {
            $name.iter().for_each(|x| assert_component(x, &trace_polys));
        };
    }
    assert_vec!(add);
    assert_vec!(add_imm);
    assert_vec!(add_small);
    assert_vec!(add_small_imm);
    assert_vec!(add_ap);
    assert_vec!(add_ap_op_1_base_fp);
    assert_vec!(add_ap_imm);
    assert_vec!(assert_eq);
    assert_vec!(assert_eq_imm);
    assert_vec!(assert_eq_double_deref);
    assert_vec!(blake);
    assert_vec!(call);
    assert_vec!(call_op_1_base_fp);
    assert_vec!(call_rel);
    assert_vec!(generic);
    assert_vec!(jnz);
    assert_vec!(jnz_dst_base_fp);
    assert_vec!(jnz_taken);
    assert_vec!(jnz_taken_dst_base_fp);
    assert_vec!(jump);
    assert_vec!(jump_double_deref);
    assert_vec!(jump_rel);
    assert_vec!(jump_rel_imm);
    assert_vec!(mul);
    assert_vec!(mul_imm);
    assert_vec!(mul_small);
    assert_vec!(mul_small_imm);
    assert_vec!(qm31);
    assert_vec!(ret);
    ...
    assert_component(verify_bitwise_xor_12, &trace_polys);
...
pub fn assert_cairo_constraints(input: ProverInput, preprocessed_trace: PreProcessedTrace) {
    ...
    println!("Preprocessed trace commitment took: {:?}", elapsed);
    ...
    tree_builder.commit(channel);
    let elapsed = now.elapsed();
    println!("Base trace commitment took: {:?}", elapsed);
    ...
    tree_builder.commit(channel);
    let elapsed = now.elapsed();
    println!("Interaction trace commitment took {:?}", elapsed);
    ...
    assert_every_cairo_component(commitment_scheme.polynomials(), &components);
    let elapsed = now.elapsed();
    println!("Components commitment took: {:?}", elapsed);
}
