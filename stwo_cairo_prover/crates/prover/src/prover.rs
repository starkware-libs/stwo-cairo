...
pub mod tests {
    ...
    pub mod slow_tests {
        ...
        #[test]
        fn test_prove_cairo_verify_all_opcode_components() {
            let compiled_program_path =
                get_compiled_cairo_program("test_prove_verify_all_opcode_components");
            let input = prover_input_from_compiled_cairo_program(&compiled_program_path);
            let preprocessed_trace = PreProcessedTraceVariant::CanonicalWithoutPedersen;
            ...
            let status = Command::new("bash")
                .arg("-c")
                .arg(format!("(cd ../../../stwo_cairo_verifier; scarb execute --package stwo_cairo_verifier --no-warnings --arguments-file {} --output standard --target standalone)", proof_file.path().to_str().unwrap()))
        ...
        pub mod builtin_tests {
            ...
            fn test_prove_verify_all_builtins() {
                let compiled_program_path =
                    get_compiled_cairo_program("test_prove_verify_all_builtins");
                let input = prover_input_from_compiled_cairo_program(&compiled_program_path);
            ...
            fn test_add_mod_builtin_constraints() {
                let compiled_program_path =
                    get_compiled_cairo_program("test_prove_verify_add_mod_builtin");
                let input = prover_input_from_compiled_cairo_program(&compiled_program_path);
                assert_cairo_constraints(input, PreProcessedTrace::canonical_without_pedersen());
            }
            ...
            fn test_bitwise_builtin_constraints() {
                let compiled_program_path =
                    get_compiled_cairo_program("test_prove_verify_bitwise_builtin");
                let input = prover_input_from_compiled_cairo_program(&compiled_program_path);
            ...
            fn test_mul_mod_builtin_constraints() {
                let compiled_program_path =
                    get_compiled_cairo_program("test_prove_verify_mul_mod_builtin");
                let input = prover_input_from_compiled_cairo_program(&compiled_program_path);
                assert_cairo_constraints(input, testing_preprocessed_tree(19));
            }
            ...
            fn test_pedersen_builtin_constraints() {
                let compiled_program_path =
                    get_compiled_cairo_program("test_prove_verify_pedersen_builtin");
                let input = prover_input_from_compiled_cairo_program(&compiled_program_path);
            ...
            fn test_poseidon_builtin_constraints() {
                let compiled_program_path =
                    get_compiled_cairo_program("test_prove_verify_poseidon_builtin");
                let input = prover_input_from_compiled_cairo_program(&compiled_program_path);
                assert_cairo_constraints(input, testing_preprocessed_tree(19));
            }
            ...
            fn test_range_check_bits_96_builtin_constraints() {
                let compiled_program_path =
                    get_compiled_cairo_program("test_prove_verify_range_check_bits_96_builtin");
                let input = prover_input_from_compiled_cairo_program(&compiled_program_path);
                assert_cairo_constraints(input, testing_preprocessed_tree(19));
            }
            ...
            fn test_range_check_bits_128_builtin_constraints() {
                let compiled_program_path =
                    get_compiled_cairo_program("test_prove_verify_range_check_bits_128_builtin");
                let input = prover_input_from_compiled_cairo_program(&compiled_program_path);
                assert_cairo_constraints(input, testing_preprocessed_tree(19));
            }
        }
