    ...
    SimdBackend: BackendForChannel<MC>,
{
    ...
    // Compute the maximum log size of the trace as the maximum log size of the columns in the
    // preprocessed trace and the log sizes in the claim.
    ...
    let max_domain_size = max_log_trace_size
        + std::cmp::max(
            cairo_air_log_degree_bound,
            pcs_config.fri_config.log_blowup_factor,
        );
    if max_domain_size > MAX_CANONICAL_COSET_LOG_SIZE {
        panic!(
            "Max domain size must be less than or equal to the maximum canonical coset log size"
        );
    }
    if let Some(lifting_log_size) = pcs_config.lifting_log_size {
        if lifting_log_size < max_domain_size {
