...
mod test_builtin_segments {
    ...
    fn initialize_memory(begin_addr: usize, instance_example: &[u128]) -> MemoryBuilder {
        ...
        for (i, &value) in instance_example.iter().enumerate() {
            ...
            memory_builder.set((begin_addr + i) as u64, memory_value);
        }
        memory_builder
    }
    ...
    fn test_fill_builtin_segment() {
        ...
        let mut memory_builder = initialize_memory(begin_addr, &instance_example);
        ...
        for instance in num_instances..new_num_instances {
            for j in 0..cells_per_instance {
                ...
                assert!(memory.address_to_id[address] == memory.address_to_id[begin_addr + j]);
            }
        }
    }
}
