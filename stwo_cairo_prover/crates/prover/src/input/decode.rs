...
impl OpcodeExtension {
    ...
    /// an OpcodeExtension x and returns Some(x).
    /// Panics if the trailing bits do not correspond to a valid OpcodeExtension.
    pub fn from_instruction_trailing_bits(value: u128) -> Option<OpcodeExtension> {
        match value {
            0 => Some(OpcodeExtension::Stone),
            1 => Some(OpcodeExtension::Blake),
            2 => Some(OpcodeExtension::BlakeFinalize),
