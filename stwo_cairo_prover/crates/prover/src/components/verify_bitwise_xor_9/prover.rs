...
impl ClaimGenerator {
        ...
        SimdBackend: BackendForChannel<MC>,
    {
        ...
        (Claim::new(), InteractionClaimGenerator { lookup_data })
    }
...
fn write_trace_simd(mults: Vec<PackedM31>) -> (ComponentTrace<N_TRACE_COLUMNS>, LookupData) {
    ...
    let preprocessed_xor_column_0 = BitwiseXor::new(N_BITS, 0);
    let preprocessed_xor_column_1 = BitwiseXor::new(N_BITS, 1);
    let preprocessed_xor_column_2 = BitwiseXor::new(N_BITS, 2);
    ...
    trace
        ...
        .for_each(|((row_index, row), lookup_data)| {
            ...
            *lookup_data.verify_bitwise_xor_9_0 = [
                preprocessed_xor_column_0.packed_at(row_index),
                preprocessed_xor_column_1.packed_at(row_index),
                preprocessed_xor_column_2.packed_at(row_index),
            ];
...
impl InteractionClaimGenerator {
    ...
    where
        SimdBackend: BackendForChannel<MC>,
    {
        assert!(self.lookup_data.verify_bitwise_xor_9_0.len() == 1 << (LOG_SIZE - LOG_N_LANES));
