...
pub struct RangeCheckClaimProver<const RC_LOG_HEIGHT: u32, const N_REPETITIONS: usize> {
    pub multiplicities: Vec<Vec<PackedUInt32>>,
}
    ...
    RangeCheckClaimProver<RC_LOG_HEIGHT, N_REPETITIONS>
{
    ...
    pub fn add_inputs(&mut self, inputs: &[M31]) {
        for input in inputs {
    ...
    pub fn generate_trace(&mut self) -> Vec<BaseColumn> {
        let mut trace = (0..(N_REPETITIONS + 1))
            .map(|_| Col::<SimdBackend, BaseField>::zeros(1 << RC_LOG_HEIGHT))
            .collect_vec();
    ...
    pub fn write_trace(
        ...
        let multiplicities = (0..N_REPETITIONS)
            .map(|i| trace[TRACE_MULTIPLICITIES_OFFSET + i].data.clone())
            .collect_vec();
...
impl<const RC_LOG_HEIGHT: u32, const N_REPETITIONS: usize> Default
    for RangeCheckClaimProver<RC_LOG_HEIGHT, N_REPETITIONS>
{
    fn default() -> Self {
        Self::new()
    }
...
pub struct RangeCheckInteractionClaimProver<const RC_LOG_HEIGHT: u32, const N_REPETITIONS: usize> {
    pub multiplicities: Vec<Vec<PackedM31>>,
}
impl<const RC_LOG_HEIGHT: u32, const N_REPETITIONS: usize>
    RangeCheckInteractionClaimProver<RC_LOG_HEIGHT, N_REPETITIONS>
{
    pub fn with_capacity(capacity: usize) -> Self {
        Self {
            multiplicities: vec![Vec::with_capacity(capacity); N_REPETITIONS],
        }
    }
    ...
    pub fn generate_interaction_trace(
        ...
        for vec_row in 0..(1 << (RC_LOG_HEIGHT - LOG_N_LANES)) {
            ...
            for rep in 0..N_REPETITIONS {
                ...
                let num: PackedQM31 = (-self.multiplicities[rep][vec_row]).into();
