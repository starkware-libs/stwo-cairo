...
impl ClaimGenerator {
    pub fn new(memory: &Memory) -> Self {
        // Note that `memory.address_to_id` starts from address 0, but because 0 is reserved, we
        // ignore the first element, and collect the addresses starting from 1. In the trace itself,
        // the addresses are offset by 1, so row `i`` in the trace corresponds to address `i+1`.
