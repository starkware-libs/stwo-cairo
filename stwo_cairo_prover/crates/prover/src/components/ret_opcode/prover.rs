...
impl SubComponentInputs {
    ...
    unsafe fn uninitialized(len: usize) -> Self {
        ...
        unsafe {
            s.memory_address_to_id_inputs.iter().for_each(|vec| {
                (*vec.get()).set_len(len);
            });
            s.memory_id_to_big_inputs.iter().for_each(|vec| {
                (*vec.get()).set_len(len);
            });
            s.verify_instruction_inputs.iter().for_each(|vec| {
                (*vec.get()).set_len(len);
            });
        }
        s
...
impl LookupData {
    ...
    unsafe fn uninitialized(len: usize) -> Self {
        ...
        unsafe {
            s.memoryaddresstoid.iter().for_each(|vec| {
                (*vec.get()).set_len(len);
            });
            s.memoryidtobig.iter().for_each(|vec| {
                (*vec.get()).set_len(len);
            });
            s.opcodes.iter().for_each(|vec| {
                (*vec.get()).set_len(len);
            });
            s.verifyinstruction.iter().for_each(|vec| {
                (*vec.get()).set_len(len);
            });
        }
