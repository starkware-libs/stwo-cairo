...
impl SubComponentInputs {
    ...
    fn with_capacity(capacity: usize) -> Self {
        Self {
            ...
            verify_instruction_inputs: [UnsafeCell::new(Vec::with_capacity(capacity))],
        }
    }

    unsafe fn set_len(&self, len: usize) {
        self.memory_address_to_id_inputs.iter().for_each(|vec| {
            (*vec.get()).set_len(len);
        });
        self.memory_id_to_big_inputs.iter().for_each(|vec| {
            (*vec.get()).set_len(len);
        });
        self.verify_instruction_inputs.iter().for_each(|vec| {
            (*vec.get()).set_len(len);
        });
    }
...
pub fn write_trace_simd(
    ...
    const N_TRACE_COLUMNS: usize = 11;
    let trace = Arc::new(ThreadSafeTrace::<N_TRACE_COLUMNS>::new(
        inputs.len() * N_LANES,
    ));
    let lookup_data = Arc::new(LookupData::with_capacity(inputs.len()));
    let sub_components_inputs = Arc::new(SubComponentInputs::with_capacity(inputs.len() * N_LANES));
    unsafe {
        lookup_data.set_len(inputs.len());
        sub_components_inputs.set_len(inputs.len() * N_LANES);
    }
                ...
                (*sub_components_inputs.verify_instruction_inputs[0].get())
                    ...
                    .copy_from_slice(
                        &(
                            ...
                            [
                                M31_1, M31_1, M31_0, M31_1, M31_0, M31_0, M31_0, M31_1, M31_0,
                                M31_0, M31_0, M31_0, M31_0, M31_1, M31_0,
                            ],
                        )
                            .unpack(),
                    );
...
impl LookupData {
    ...
    fn with_capacity(capacity: usize) -> Self {
        Self {
            ...
            verifyinstruction: [Vec::with_capacity(capacity).into()],
        }
    }

    unsafe fn set_len(&self, len: usize) {
        self.memoryaddresstoid.iter().for_each(|vec| {
            (*vec.get()).set_len(len);
        });
        self.memoryidtobig.iter().for_each(|vec| {
            (*vec.get()).set_len(len);
        });
        self.opcodes.iter().for_each(|vec| {
            (*vec.get()).set_len(len);
        });
        self.verifyinstruction.iter().for_each(|vec| {
            (*vec.get()).set_len(len);
        });
    }
}
