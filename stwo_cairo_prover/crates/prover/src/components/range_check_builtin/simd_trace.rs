...
impl RangeCheckClaimProver {
    pub fn new(input: SegmentAddrs) -> Self {
        ...
        let size = addresses.len().next_power_of_two().max(64);
        addresses.resize(size, 0);

        let inputs = addresses
            .into_iter()
            .array_chunks::<N_LANES>()
            .map(|chunk| RangeCheckInput {
                address: PackedM31::from_array(std::array::from_fn(|i| {
                    M31::from_u32_unchecked(chunk[i])
                })),
            })
            .collect_vec();
        Self { inputs }
    ...
    pub fn write_trace(
        ...
        let claim = RangeCheckClaim {
            ...
            n_values: self.inputs.len() * N_LANES,
        };
...
pub struct RangeCheckInteractionProver {
    pub memory_inputs: Vec<PackedM31>,
    pub memory_outputs: Vec<[PackedM31; N_VALUES_FELTS]>,
}

impl RangeCheckInteractionProver {
    pub fn with_capacity(capacity: usize) -> Self {
        Self {
            memory_inputs: Vec::with_capacity(capacity),
            memory_outputs: Vec::with_capacity(capacity),
        }
    }
    ...
    pub fn write_interaction_trace(
        ...
        let log_size = self.memory_inputs.len().ilog2() + LOG_N_LANES;
...
pub fn write_trace_simd(
    ...
    let address_initial_offset =
        PackedM31::broadcast(BaseField::from(inputs[0].address.into_simd()[0]));
    ...
    for vec_row in 0..(1 << (log_size - LOG_N_LANES)) {
        ...
        let split_values: [PackedM31; N_VALUES_FELTS] = memory_trace_generator
            .deduce_output(row_input.address)[..N_VALUES_FELTS]
            .try_into()
            .unwrap();
        let address = row_input.address - address_initial_offset;
        trace[0].data[vec_row] = address;
        for (i, v) in split_values.iter().enumerate() {
            trace[i + 1].data[vec_row] = *v;
        }

        interaction_prover.memory_inputs.push(row_input.address);
        interaction_prover.memory_outputs.push(split_values);
    }
...
pub fn gen_interaction_trace(
    ...
    for vec_row in 0..(1 << (log_size - LOG_N_LANES)) {
        let p_mem: PackedQM31 = memory_lookup_elements.combine(
            &chain!(
                [interaction_prover.memory_inputs[vec_row]],
                interaction_prover.memory_outputs[vec_row]
            )
            .collect_vec(),
        );
        let p_rc2: PackedQM31 = range2_lookup_elements
            .combine(&[interaction_prover.memory_outputs[vec_row][N_VALUES_FELTS - 1]]);
...
mod tests {
    ...
    fn test_generate_trace() {
        ...
        for output in &interaction_prover.memory_outputs {
        ...
        assert_eq!(
            (1 + interaction_prover.memory_outputs[0].len()) * N_LANES,
            1 << log_size
        );
        for (i, addresses) in interaction_prover.memory_inputs.iter().enumerate() {
            assert_eq!(
                addresses.to_array(),
