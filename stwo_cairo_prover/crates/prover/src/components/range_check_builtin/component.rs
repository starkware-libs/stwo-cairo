...
impl RangeCheckClaim {
    ...
    pub fn log_sizes(&self) -> TreeVec<Vec<u32>> {
        let log_size = self.n_values.next_power_of_two().ilog2();
        let interaction_0_log_sizes = vec![log_size; N_ADDRESS_FELTS + N_VALUES_FELTS];
        let interaction_1_log_sizes = vec![log_size; SECURE_EXTENSION_DEGREE];
