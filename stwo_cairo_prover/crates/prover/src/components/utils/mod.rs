...
///
/// TODO(Ohad): used for testing, remove.
#[cfg(test)]
pub fn sparse_m31_representation<const N: usize>(splits: [u32; N], value: u32) -> [M31; N] {
    ...
    let masks: [_; N] = std::array::from_fn(|i| ((1 << splits[i]) - 1) << suffix_sum[i]);
...
mod tests {
    ...
    fn test_packed_sparse_enumerate() {
        ...
        let expected_column = |log_n_reps: u32, n_bits: u32, n_trailing_bits: usize| -> Vec<M31> {
            std::iter::repeat(
                (0..1 << n_bits)
                    .flat_map(|i| vec![M31(i); 1 << n_trailing_bits])
                    .collect_vec(),
            )
            .take(1 << log_n_reps)
            .flatten()
        ...
        let expected_0 = expected_column(0, 5, 6);
        let expected_1 = expected_column(5, 3, 3);
        let expected_2 = expected_column(8, 3, 0);
        let expected = [expected_0, expected_1, expected_2];
