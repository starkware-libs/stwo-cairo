...
use crate::components::utils::{
    dense_representation, excluded_leading_sum, excluded_trailing_sum,
    generate_expanded_enumeration, packed_sparse_representation,
};
...
impl<const N: usize> RangeCheckClaimProver<N> {
    pub fn new(log_ranges: [u32; N]) -> Self {
        let trailing_sums = excluded_trailing_sum(log_ranges);
        let multiplicities =
    ...
    pub fn add_m31(&mut self, input: [M31; N]) {
        let num = dense_representation(self.trailing_sums, input) as usize;
    ...
    fn write_fixed_columns(&self) -> [BaseColumn; N] {
        let trailing_bits = excluded_trailing_sum(self.log_ranges);
        let leading_bits = excluded_leading_sum(self.log_ranges);
        std::array::from_fn(|i| {
            let (leading, n_bits, trailing) =
                (leading_bits[i], self.log_ranges[i], trailing_bits[i]);
            BaseColumn {
...
impl<const N: usize> RangeCheckInteractionClaimProver<N> {
    ...
    pub fn write_interaction_trace(
        ...
        let trailing_sums = excluded_trailing_sum(self.log_ranges);
        ...
        for vec_row in 0..(1 << (log_size - LOG_N_LANES)) {
            ...
            let denom = lookup_elements
                .combine(&packed_sparse_representation(trailing_sums, vec_row as u32));
...
mod tests {
    ...
    use crate::components::utils::{excluded_trailing_sum, sparse_representation};
    ...
    fn test_prove() {
        ...
        let inputs: [[M31; 3]; 30] = std::array::from_fn(|_| {
            sparse_representation(trailing_sum, rng.gen::<u32>() % (1 << LOG_HEIGHT))
        });
