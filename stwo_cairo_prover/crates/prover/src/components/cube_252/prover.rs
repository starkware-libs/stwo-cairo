...
pub struct ClaimGenerator {
    pub inputs: DashMap<InputType, u32>,
}
impl ClaimGenerator {
    pub fn new() -> Self {
        Self {
        ...
        SimdBackend: BackendForChannel<MC>,
    {
        ...
        let (mut inputs, mults): (Vec<InputType>, Vec<u32>) = self.inputs.into_iter().unzip();
        let mults = BaseColumn::from_iter(mults.into_iter().map(M31)).data;
    ...
    pub fn add_input(&mut self, input: &InputType) {
        if let Some(mut input) = self.inputs.get_mut(input) {
            *input += 1;
        } else {
            self.inputs.insert(*input, 1);
        }
    }
