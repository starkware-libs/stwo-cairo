...
impl ClaimGenerator {
    ...
    pub fn new() -> Self {
        Self {
            // correct?
            mults: AtomicMultiplicityColumn::new(PEDERSEN_TABLE_N_ROWS.next_power_of_two()), /* 2**23 PEDERSEN_TABLE_N_ROWS.next_power_of_two() */
    ...
    trace
        ...
        .for_each(|((row_index, mut row), lookup_data)| {
            let pedersen_points_table: [PackedM31; PEDERSEN_TABLE_N_COLUMNS] =
                std::array::from_fn(|i| pedersen_points_table[i].packed_at(row_index)); // packed_at? create consts in original table?
