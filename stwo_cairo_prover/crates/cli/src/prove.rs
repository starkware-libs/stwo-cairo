...
use cairo_prover_cli::utils::{create_file, read_to_string, run_binary, IoErrorWithPath};
use clap::Parser;
use serde::Serialize;
...
#[derive(Parser, Debug)]
struct Args {
    // TODO(Ohad): change to the new adapter flow.
    ...
    <MC::H as MerkleHasher>::Hash: CairoSerialize,
{
    ...
    match proof_format {
        ...
        ProofFormat::CairoSerde => {
            ...
            let proof_str = serde_json::to_string_pretty(&hex_strings)?;
            proof_file.write_all(proof_str.as_bytes()).unwrap();
        }
    }
