...
use crate::serde_utils::deserialize_cairo1_proof;
    ...
    match proof_format {
        ...
        ProofFormat::CairoSerde => {
            ...
            let felts: Vec<starknet_ff::FieldElement> =
                json::from_str(&proof_str).map_err(std::io::Error::other)?;
            Ok(deserialize_cairo1_proof(
                &mut felts.iter(),
                preprocessed_variant
...
pub fn unsort_and_transpose_queried_values(
    ...
    let mut new_queried_values_per_tree = vec![];
    // Transpose the preprocessed queried values. The preprocessed columns are already sorted in
    // ascending order by size so we don't need to compute the permutation.
    let queried_values = queried_values_per_tree.first().unwrap();
    let n_cols = n_preprocessed_cols;
    let mut new_queried_values = vec![];
    for i in 0..n_cols {
    ...
    // Transpose and permute the queried values for base trace and interaction trace.
    for (queried_values, col_sizes) in queried_values_per_tree
        [ORIGINAL_TRACE_IDX..=INTERACTION_TRACE_IDX]
        .iter()
        .zip_eq(
            trace_and_interaction_trace_log_sizes[ORIGINAL_TRACE_IDX..=INTERACTION_TRACE_IDX]
                .iter(),
    ...
    // Transpose and permute the queried values for the composition polynomial commitment. All the
    // columns have the same size so we don't need to compute the permutation. The number of
    // columns in this tree is 2^{COMPOSITION_LOG_SPLIT} * 4.
