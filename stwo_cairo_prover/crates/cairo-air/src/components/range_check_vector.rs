    ...
    ([$($log_range:expr),+], $suffix:ident, $suffix_upper:ident) => {
        paste::paste! {
            pub mod [<range_check_$($log_range) _ $suffix>] {
                $crate::range_check_eval!($($log_range),+, $suffix, $suffix_upper);
            }
        }
    };
}
...
macro_rules! range_check_eval{
    ($($log_range:expr),+, $suffix:ident, $suffix_upper:ident) => {
        paste::paste! {
                ...
                pub struct Eval {
                    pub claim: Claim,
                    pub [<range_check_$($log_range) _ $suffix>]_lookup_elements: relations::[<RangeCheck_$($log_range) _ $suffix_upper>],
                }
                impl Eval {<RangeCheck_$($log_range) _ $suffix_upper>
                    pub fn new(lookup_elements: relations::[<RangeCheck_$($log_range) _ $suffix_upper>],) -> Self {
                        Self {
                            lookup_elements,
                        }
                    }
