    ...
    ([$($log_range:expr),+], $suffix:ident, $suffix_upper:ident) => {
        paste::paste! {
            pub mod [<range_check_$($log_range)_*_$suffix>] {
                $crate::range_check_eval!($suffix, $suffix_upper, $($log_range),+);
            }
        }
    };
}
...
macro_rules! range_check_eval{
    ($suffix:ident, $suffix_upper:ident, $($log_range:expr),+) => {
        paste::paste! {
            ...
            pub struct Eval {
                pub lookup_elements: relations::[<RangeCheck_$($log_range)_*_$suffix_upper>],
            }
            impl Eval {
                pub fn new(lookup_elements: relations::[<RangeCheck_$($log_range)_*_$suffix_upper>],) -> Self {
                    Self {
                        lookup_elements,
                    }
                }
            ...
            mod tests {
                ...
                use super::*;
                use $crate::components::constraints_regression_test_values::[<RANGE_CHECK_$($log_range)_*_$suffix_upper>];

                #[test]
                fn [<constraints_regression_range_check_$($log_range)_*_$suffix>]() {
                    let mut rng = SmallRng::seed_from_u64(0);
                    let eval = Eval {
                        lookup_elements: relations::[<RangeCheck_$($log_range)_*_$suffix_upper>]::dummy(),
                    };
                    ...
                    assert_eq!(sum, [<RANGE_CHECK_$($log_range)_*_$suffix_upper>]);
                }
