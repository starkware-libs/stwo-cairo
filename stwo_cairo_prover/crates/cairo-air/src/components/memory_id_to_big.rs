...
pub type BigComponent = FrameworkComponent<Eval>;
...
pub struct Component {
    pub components: Vec<BigComponent>,
}
...
impl Component {
    pub fn new(
        ...
        // Every component is responsible for a range of memory ids. The ids must not overlap. Use
        // an offset to keep track of the previous component's range.
        let mut components = vec![];
        let mut offset = 0;
        for (&log_size, &claimed_sum) in log_sizes.iter().zip_eq(claimed_sums) {
            components.push(BigComponent::new(
                tree_span_provider,
                Eval::new(log_size, offset, common_lookup_elements.clone()),
                claimed_sum,
            ));
            offset += 1 << log_size;
        }
    ...
    pub fn components(&self) -> Vec<&dyn StwoComponent> {
        self.components
            .iter()
            .map(|component| component as &dyn StwoComponent)
            .collect()
    }
}
