...
//     ) -> Self { Self { log_n_rows, offset, common_lookup_elements, }
//     }
// }
...
// ) -> Vec<BigComponent> { // Every component is responsible for a range of memory ids. The ids
//   must not overlap. Use an // offset to keep track of the previous component's range. let mut
//   components = vec![]; let mut offset = 0; for (&log_size, &claimed_sum) in
//   log_sizes.iter().zip_eq(claimed_sums) { components.push(BigComponent::new( tree_span_provider,
//   BigEval::new(log_size, offset, common_lookup_elements.clone()), claimed_sum, )); offset += 1 <<
//   log_size; } components
// }
...
impl Claim {
    pub fn log_sizes(&self) -> TreeVec<Vec<u32>> {
        ...
        let trace_log_sizes = small_trace_log_sizes;
        ...
        let interaction_log_sizes = small_interaction_log_sizes;

        TreeVec::new(vec![vec![], trace_log_sizes, interaction_log_sizes])
    }

    pub fn mix_into(&self, channel: &mut impl Channel) {
        chain!([self.small_log_size]).for_each(|log_size| channel.mix_u64(log_size as u64));
    }
}
...
mod tests {
    ...
    use super::*;
    use crate::components::constraints_regression_test_values::SMALL_MEMORY_ID_TO_BIG;
