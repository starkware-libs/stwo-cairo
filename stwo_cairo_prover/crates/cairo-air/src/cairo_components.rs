...
use crate::components::{
    ...
    memory_address_to_id, memory_id_to_big, mul_mod_builtin, mul_opcode, mul_opcode_small,
...
pub struct CairoComponents {
    ...
    pub memory_id_to_big: Option<memory_id_to_big::Component>,
...
impl CairoComponents {
    pub fn new(
        ...
        let memory_id_to_big_component =
            cairo_claim
                .memory_id_to_big
                .as_ref()
                .map(|memory_id_to_big| {
                    memory_id_to_big::Component::new(
                        tree_span_provider,
                        memory_id_to_big,
    ...
    pub fn components(&self) -> Vec<&dyn Component> {
        ...
        if let Some(component) = &self.memory_id_to_big {
            vec.extend(component.components());
        }
...
impl Display for CairoComponents {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        if let Some(ref memory_id_to_value) = self.memory_id_to_big {
            for component in &memory_id_to_value.big_components {
                writeln!(
                    f,
                    "MemoryIdToValue: {}",
                    indented_component_display(component)
            ...
            writeln!(
                ...
                indented_component_display(&memory_id_to_value.small_component)
            )?;
        }
...
pub fn cairo_provers(components: &CairoComponents) -> Vec<&dyn ComponentProver<SimdBackend>> {
    ...
    if let Some(component) = &components.memory_id_to_big {
        vec.extend(component.provers());
    }
...
pub fn cairo_relation_entries(
    ...
    if let Some(memory_component) = &cairo_components.memory_id_to_big {
        for component in &memory_component.big_components {
            entries.extend(add_to_relation_entries(component, trace));
        }
        entries.extend(add_to_relation_entries(
            &memory_component.small_component,
            trace,
        ));
    }
...
pub fn assert_cairo_components(trace: TreeVec<Vec<&Vec<M31>>>, cairo_components: &CairoComponents) {
    ...
    if let Some(memory_component) = &cairo_components.memory_id_to_big {
        ...
        assert_component(&memory_component.small_component, &trace);
    }
    if let Some(component) = &cairo_components.range_check_6 {
        assert_component(component, &trace);
    }
