...
impl CairoClaim {
    ...
    pub fn log_sizes(&self) -> TreeVec<Vec<u32>> {
        let log_sizes_list = vec![
            ...
            TreeVec::concat_cols(chain!(
                self.blake_round.iter().map(|c| c.log_sizes()),
                self.blake_g.iter().map(|c| c.log_sizes()),
                self.blake_sigma.iter().map(|c| c.log_sizes()),
                self.triple_xor_32.iter().map(|c| c.log_sizes()),
                self.verify_bitwise_xor_12.iter().map(|c| c.log_sizes()),
            )),
...
impl CairoInteractionClaim {
    pub fn mix_into(&self, channel: &mut impl Channel) {
        self.add.as_ref().inspect(|c| c.mix_into(channel));
        self.add_small.as_ref().inspect(|c| c.mix_into(channel));
        self.add_ap.as_ref().inspect(|c| c.mix_into(channel));
        self.assert_eq.as_ref().inspect(|c| c.mix_into(channel));
        self.assert_eq_imm.as_ref().inspect(|c| c.mix_into(channel));
        self.assert_eq_double_deref
            .as_ref()
            .inspect(|c| c.mix_into(channel));
        self.blake.as_ref().inspect(|c| c.mix_into(channel));
        self.call.as_ref().inspect(|c| c.mix_into(channel));
        self.call_rel_imm.as_ref().inspect(|c| c.mix_into(channel));
        self.generic.as_ref().inspect(|c| c.mix_into(channel));
        self.jnz.as_ref().inspect(|c| c.mix_into(channel));
        self.jnz_taken.as_ref().inspect(|c| c.mix_into(channel));
        self.jump.as_ref().inspect(|c| c.mix_into(channel));
        self.jump_double_deref
            .as_ref()
            .inspect(|c| c.mix_into(channel));
        self.jump_rel.as_ref().inspect(|c| c.mix_into(channel));
        self.jump_rel_imm.as_ref().inspect(|c| c.mix_into(channel));
        self.mul.as_ref().inspect(|c| c.mix_into(channel));
        self.mul_small.as_ref().inspect(|c| c.mix_into(channel));
        self.qm31.as_ref().inspect(|c| c.mix_into(channel));
        self.ret.as_ref().inspect(|c| c.mix_into(channel));
