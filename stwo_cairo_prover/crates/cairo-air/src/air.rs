...
mod tests {
    ...
    // Test the standalone function accumulate_relation_uses.
    #[test]
    fn test_accumulate_relation_uses() {
        let mut relation_counts = HashMap::new();
        // 2 rows, each row has 1 use of relation_1, so 2 uses in total.
        let relation_uses_per_row = [RelationUse {
            relation_id: "relation_1",
            uses: 1,
        }];
        let log_size = 1;
        accumulate_relation_uses(&mut relation_counts, relation_uses_per_row, log_size);
        assert_eq!(relation_counts.get("relation_1"), Some(&2));

        // 4 rows, therefore additional 8 uses of relation_1 and 16 of relation_2.
        let log_size = 2;
        ...
        accumulate_relation_uses(&mut relation_counts, relation_uses_per_row, log_size);
        assert_eq!(relation_counts.get("relation_1"), Some(&10));
        assert_eq!(relation_counts.get("relation_2"), Some(&16));
    }
}
