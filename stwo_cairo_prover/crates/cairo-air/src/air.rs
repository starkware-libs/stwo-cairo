...
use crate::cairo_claim::{CairoClaim, CairoInteractionClaim};
...
pub fn accumulate_relation_memory(
    relation_uses: &mut RelationUsesDict,
    memory_id_to_value: &Option<memory_id_to_big::Claim>,
) {
    let claim = memory_id_to_value
        .as_ref()
        .expect("memory_id_to_value must be Some");
