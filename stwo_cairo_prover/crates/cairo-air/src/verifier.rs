...
fn verify_builtins(builtins_claim: &BuiltinsClaim, segment_ranges: &PublicSegmentRanges) {
    ...
    // The macro has two patterns:
    // 1. The generic version: it takes just the builtin name and computes all that is needed.
    // 2. The "range_check" version, which takes two paramters: a) name, like the generic version.
    //    b) a prefix for the segment start field.
    macro_rules! check_builtin_generic {
        // Range check branch: when the pattern is not the default (because of range_check).
        (
            $name:ident,
            $start_field_prefix:ident
        ) => {
            paste! {
                check_builtin(
                    builtins_claim.[<$name _builtin>]
                        .map(|claim| BuiltinClaim {
                            segment_start: claim.[<$start_field_prefix _builtin_segment_start>],
        ...
        // Generic branch: only one identifier provided.
        ($name:ident) => {
            paste! {
                check_builtin(
                    builtins_claim.[<$name _builtin>]
    ...
    check_builtin_generic!(range_check_128, range_check);
    check_builtin_generic!(range_check_96, range_check96);
