...
use stwo_cairo_common::prover_types::cpu::{CasmState, FELT252_BITS_PER_WORD, FELT252_N_WORDS, M31};
...
impl FlatClaim {
    ...
    pub fn mix_into(&self, channel: &mut impl Channel) {
        channel.mix_felts(&pack_into_secure_felts(enable_bits_to_u32s(&self.component_enable_bits).into_iter()));
        channel.mix_felts(&pack_into_secure_felts(self.component_log_sizes.iter().cloned()));
        channel.mix_felts(&pack_into_secure_felts(public_data_to_u32s(&self.public_data).into_iter()));
    }
}
