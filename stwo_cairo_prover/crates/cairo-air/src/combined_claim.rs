...
use crate::air::{CairoClaim, CairoInteractionClaim, PublicData, PublicMemory};
...
fn get_public_claim(claim: &CairoClaim) -> (Vec<BaseField>, Vec<bool>, Vec<u32>) {
    ...
    let PublicData {
        public_memory:
            PublicMemory {
                program,
                // TODO(Gali): Add either public segments or public parameters in builtinsto the
                // public claim
                public_segments: _,
                output,
                safe_call_ids,
    ...
    }) = range_check_128_builtin
    {
        ...
        component_enable_bits.push(true);
        public_claim.push(BaseField::from_u32_unchecked(
            *range_check_builtin_segment_start,
        ));
    } else {
    ...
    for log_size in big_log_sizes {
        // TODO(Gali): How many big log sizes are there?
        component_log_sizes.push(*log_size);
        component_enable_bits.push(true);
    }
...
fn single_log_size<T>(
    ...
    if log_sizes.is_empty() {
        component_log_sizes.push(0_u32);
        component_enable_bits.push(false);
    } else if let [log_size] = log_sizes {
        component_log_sizes.push(get_log_size(log_size));
        component_enable_bits.push(true);
    } else {
