...
impl<const B: usize, const L: usize> BigUInt<B, L> {
    pub fn from_felt252_array(mod_words: Vec<Felt252>) -> Self {
        ...
        let needed_limbs_per252 = (MOD_BUILTIN_WORD_BIT_LEN + 63) / 64;
        let inner_limbs_lengths = (0..needed_limbs_per252)
            .map(|i| {
                if i == (needed_limbs_per252 - 1) {
                    MOD_BUILTIN_WORD_BIT_LEN & 0x3F
                } else {
                    64
                }
            })
