...
// Length of each infra side limb in bits.
pub const BIGUINT_BITS_PER_M31: usize = 12;

#[derive(Copy, Clone, Debug, Eq, PartialEq, Hash)]
// B is the numver of bits in the BigUInt.
// L is the number of 64 bit UInt64 limbs in the BigUInt.
// F is the number of BIGUINT_BITS_PER_M31 bit M31 limbs in the BigUInt.
...
impl<const B: usize, const L: usize, const F: usize> BigUInt<B, L, F> {
    ...
    // Given an index, returns the child of index `index` of the partition into limbs of
    // BIGUINT_BITS_PER_M31 bits as a M31 felt.
    pub fn get_m31(&self, index: usize) -> M31 {
        let mask = (1u64 << BIGUINT_BITS_PER_M31) - 1;
        let child_start = BIGUINT_BITS_PER_M31 * index;
        let low_limb = child_start / 64;
        let shift_low = child_start & 0x3F;
        let high_limb = min((child_start + BIGUINT_BITS_PER_M31 - 1) / 64, L - 1);
        let value = if low_limb == high_limb {
            ((self.limbs[low_limb] >> (shift_low)) & mask) as u32
        } else {
            (((self.limbs[low_limb] >> (shift_low)) | (self.limbs[high_limb] << (64 - shift_low)))
