...
pub mod tests {
    ...
    #[test]
    fn test_packed_at_bitwise_and() {
        let bitwise_a = BitwiseAnd::new(LOG_SIZE, 0);
        let bitwise_b = BitwiseAnd::new(LOG_SIZE, 1);
        let bitwise_and = BitwiseAnd::new(LOG_SIZE, 2);
        ...
        let res_and = bitwise_and.packed_at(index / N_LANES).to_array()[index % N_LANES];

        assert_eq!(res_a.0, a as u32);
        assert_eq!(res_b.0, b as u32);
        assert_eq!(res_and.0, expected_and as u32);
    }
}
