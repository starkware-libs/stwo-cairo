...
fn create_low_section(bits_per_window: usize, point: &ProjectivePoint) -> Vec<SimpleAffinePoint> {
    let num_windows = 252 / bits_per_window;
...
fn create_high_section(
    ...
    for _ in 0..((num_windows - 1) * bits_per_window) {
...
fn create_table_rows(bits_per_window: usize) -> Vec<SimpleAffinePoint> {
    assert_eq!(252 % bits_per_window, 0);
    let num_windows = 252 / bits_per_window;
    let mut rows = vec![];
    let points = [
    ...
    rows.extend(create_low_section(bits_per_window, points[0]));
    rows.extend(create_high_section(bits_per_window, points[0], points[1]));
    rows.extend(create_low_section(bits_per_window, points[2]));
    rows.extend(create_high_section(bits_per_window, points[2], points[3]));
