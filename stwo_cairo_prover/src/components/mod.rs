...
mod tests {
    ...
    use super::ret_opcode::simd_trace::{RetOpcodeComponent, RetOpcodeSimdTraceGenerator};
    use crate::components::memory::component::{MemoryComponent, N_M31_IN_FELT252};
    use crate::components::memory::simd_trace::MemorySimdTraceGenerator;
    use crate::components::ret_opcode::simd_trace::RetOpcodeInteractionGenerator;
    use crate::prover_types::PackedUInt32;

    pub fn generate_test_simd_ret() -> RetOpcodeSimdTraceGenerator {
        let inputs = (0..8)
            .map(|i| {
                [
            ...
            .collect_vec();
        RetOpcodeSimdTraceGenerator { inputs }
    }

    pub fn generate_test_simd_ret_memory() -> MemorySimdTraceGenerator {
        let mut value = [PackedM31::zero(); N_M31_IN_FELT252];
        value[0] = PackedM31::one();

        MemorySimdTraceGenerator {
            values: vec![value; 8],
            multiplicities: vec![PackedUInt32::broadcast(0); 8],
        }
    }

    pub struct CairoTestAir {
        pub ret_component: RetOpcodeComponent,
        pub memory_component: MemoryComponent,
    }
    impl Air for CairoTestAir {
        fn components(&self) -> Vec<&dyn Component> {
            vec![&self.ret_component, &self.memory_component]
        }
    }
    impl AirProver<SimdBackend> for CairoTestAir {
