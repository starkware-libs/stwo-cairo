...
use super::component::{
    MemoryClaim, MemoryInteractionClaim, LOG_MEMORY_ADDRESS_BOUND_D, MULTIPLICITY_COLUMN_OFFSET,
    N_M31_IN_FELT252,
};
use super::MemoryLookupElements;
use crate::prover_types::PackedUInt32;

pub const LOG_SIMD_MEMORY_ADDRESS_BOUND: u32 = 7;
...
impl MemoryClaimProver {
    ...
    pub fn write_trace(
        ...
        // Lookup data.
        let adsresses_and_values = std::array::from_fn(|i| trace[i].data.clone());
        let multiplicities = trace[MULTIPLICITY_COLUMN_OFFSET].data.clone();

        // Extend trace.
        let domain = CanonicCoset::new(LOG_MEMORY_ADDRESS_BOUND_D + LOG_N_LANES).circle_domain();
        let trace = trace
            .into_iter()
        ...
        (
            ...
            InteractionClaimProver {
                adresses_and_values: adsresses_and_values,
                multiplicities,
            },
        )
    }
