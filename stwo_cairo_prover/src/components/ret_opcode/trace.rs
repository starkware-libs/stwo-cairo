...
fn write_trace_row(
    ...
    let deduction_tmp_2 = memory_trace_generator.deduce_output(col0);
    // TODO(Ohad): implement and uncomment.
    // returned_inputs.memory_inputs.push((col2) - (M31::from(1)));
    let deduction_tmp_4 = memory_trace_generator.deduce_output((col2) - (M31::from(1)));
    let col3 = deduction_tmp_4[0];
    dst[3][row_index] = col3.into();
    let col4 = deduction_tmp_4[1];
    dst[4][row_index] = col4.into();
    // returned_inputs.memory_inputs.push((col2) - (M31::from(2)));
    let deduction_tmp_5 = memory_trace_generator.deduce_output((col2) - (M31::from(2)));
    let col5 = deduction_tmp_5[0];
    dst[5][row_index] = col5.into();
    let col6 = deduction_tmp_5[1];
