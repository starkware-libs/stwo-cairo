...
pub mod tests {
    ...
    use crate::components::ret_opcode::component::{
        RetOpcode, RET_COMPONENT_ID, RET_N_TRACE_CELLS,
    };
    ...
    impl AirTraceGenerator<CpuBackend> for TestRetAirGenerator {
        ...
        fn interact(
            ...
            let ret_trace = trace.iter().take(RET_N_TRACE_CELLS).collect_vec();
            let memory_trace = trace.iter().skip(RET_N_TRACE_CELLS).collect_vec();
