...
impl Memory {
    pub fn get(&self, addr: u32) -> MemoryValue {
        match self.address_to_id[addr as usize].decode() {
            ...
            MemoryValueId::U32(id) => MemoryValue::U32(self.u32_values[id]),
    ...
    pub fn set(&mut self, addr: u64, value: MemoryValue) {
        ...
        self.address_to_id[addr as usize] = EncodedMemoryValueId::encode(match value {
            ...
            MemoryValue::U32(id) => {
                let res = self.u32_values.len();
                self.u32_values.push(id);
                MemoryValueId::U32(res)
            }
            MemoryValue::U64(id) => {
...
impl EncodedMemoryValueId {
    pub fn encode(value: MemoryValueId) -> EncodedMemoryValueId {
        match value {
            ...
            MemoryValueId::U32(id) => EncodedMemoryValueId(id as u32 | 0x4000_0000),
            MemoryValueId::U64(id) => EncodedMemoryValueId(id as u32 | 0x8000_0000),
            MemoryValueId::F252(id) => EncodedMemoryValueId(id as u32 | 0xc000_0000),
        }
    }
    pub fn decode(&self) -> MemoryValueId {
        ...
        match tag {
            ...
            1 => MemoryValueId::U32(val as usize),
            2 => MemoryValueId::U64(val as usize),
            3 => MemoryValueId::F252(val as usize),
            _ => panic!("Invalid tag"),
        }
    }
}
...
impl MemoryValue {
    pub fn as_u32(&self) -> u32 {
        match self {
            MemoryValue::I16(x) => *x as u32,
            MemoryValue::U32(x) => *x,
            MemoryValue::U64(_) => panic!("Cannot convert u64 to u32"),
            MemoryValue::F252(_) => panic!("Cannot convert F252 to u32"),
        }
    }
    pub fn as_u64(&self) -> u64 {
        match self {
            MemoryValue::I16(x) => (*x).try_into().unwrap(),
            MemoryValue::U32(x) => *x as u64,
            MemoryValue::U64(x) => *x,
...
impl From<[u32; 8]> for MemoryValue {
    fn from(value: [u32; 8]) -> MemoryValue {
        ...
        if value[0] >= SMALL_VALUE_SHIFT as u32 {
            return MemoryValue::U32(value[0]);
        }
...
mod tests {
    ...
    fn test_memory() {
        ...
        assert_eq!(memory.get(5), MemoryValue::U32(1 << 24));
    }
}
