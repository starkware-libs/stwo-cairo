...
pub fn open_file(path: &Path) -> Result<File, IoErrorWithPath> {
    File::open(path).map_err(|e| IoErrorWithPath { source: e, path: path.to_path_buf() })
}
...
pub fn create_file(path: &Path) -> Result<File, IoErrorWithPath> {
    File::create(path).map_err(|e| IoErrorWithPath { source: e, path: path.to_path_buf() })
}
...
pub fn read_to_string(path: &Path) -> Result<String, IoErrorWithPath> {
    std::fs::read_to_string(path)
        .map_err(|e| IoErrorWithPath { source: e, path: path.to_path_buf() })
}
