...
/// where `n` is the domain size and `d` is the degree of the polynomial.
///
...
fn line_fft(
    ...
    while domain_i != 0 {
        ...
        let twiddles = domains[domain_i].first_half_values().span();
...
pub impl LineDomainImpl of LineDomainTrait {
    ...
    fn first_half_values(self: @LineDomain) -> Array<M31> {
...
mod tests {
    ...
    #[test]
    fn test_line_domain_iterator() {
        let domain = LineDomainImpl::new_unchecked(CosetImpl::half_odds(3));

        let domain_iter = domain.into_iter();

        let mut i = 0;
        for x in domain_iter {
            assert_eq!(x, domain.at(i));
            i += 1;
        }
    }
