...
pub impl CommitmentSchemeVerifierImpl of CommitmentSchemeVerifierTrait {
    ...
    fn verify_values(
        ...
        let (unique_column_log_sizes, mut query_positions_per_log_size) = fri_verifier
        ...
        loop {
            ...
            let mut queries_per_log_size_copy = Default::default();

            for log_size in unique_column_log_sizes {
                let log_size = (*log_size).into();
                queries_per_log_size_copy.insert(log_size, queries_per_log_size.get(log_size));
            };

            if let Result::Err(err) = tree
                .verify(queries_per_log_size_copy, queried_values, decommitment) {
        ...
        let (opening_positions_per_log_size, decomposed_fri_answers) =
            get_fri_answers_decommitment_values(
            fri_answers.span(), ref queries_per_log_size_cpy, fri_query_domain_log_sizes,
        );
        if let Result::Err(err) = quotients_tree
            .verify(
                opening_positions_per_log_size,
                decomposed_fri_answers.span(),
...
fn get_fri_answers_decommitment_values(
    ...
    for log_size in log_sizes {
        ...
        opening_positions_per_log_size
            .insert(log_size, NullableTrait::new(opening_positions.span()));
    };
