...
use crate::fri::{FOLD_FACTOR, FOLD_STEP};
...
use crate::queries::{
    ...
    get_folded_query_positions,
};
// TODO: remove 'as ArrayUtilImpl'.
...
fn fri_answers_for_log_size(
    ...
    let mut all_subcircle_evals = array![];

    for folded_position in get_folded_query_positions(query_positions, FOLD_STEP) {
        let subcircle_start = folded_position * FOLD_FACTOR;
        let subcircle_end = subcircle_start + FOLD_FACTOR;

        let mut subcircle_evals = array![];

        for eval_position in subcircle_start..subcircle_end {
            subcircle_evals
                .append(
                    if query_positions.next_if_eq(@eval_position).is_some() {
                        accumulate_row_quotients(
                            ...
                            commitment_domain.at(bit_reverse_index(eval_position, log_size)),
                        )
                    } else {
                        // TODO(andrew): Should we do error here? Should we do errors at all in the
                        // Cairo verifier and instead just panic with error message.
                        *missing_values.pop_front().unwrap()
                    },
                );
        };

        let subcircle_initial = commitment_domain
            .index_at(bit_reverse_index(subcircle_start, log_size));
        let subcircle_domain = CircleDomainImpl::new(
            CosetImpl::new(subcircle_initial, FOLD_STEP - 1),
        );
        all_subcircle_evals.append(CircleEvaluationImpl::new(subcircle_domain, subcircle_evals));
    };

    // Sanity check all rows have been visited.
    assert!(row_index_iter.next().is_none());

    Result::Ok(SparseCircleEvaluationImpl::new(all_subcircle_evals))
}
...
fn accumulate_row_quotients(
    ...
    for batch_i in 0..n_batches {
        ...
        for sample_i in 0..batch_size {
            ...
            let column_value = *column.at(row);
