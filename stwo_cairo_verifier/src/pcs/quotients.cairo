...
/// * `queried_values_by_column`: Evals of each column at the columns corresponding query positions.
...
pub fn fri_answers(
    ...
    loop {
        ...
        for column in columns {
            ...
            queried_values.append(queried_values_by_column[column]);
        };
...
fn fri_answers_for_log_size(
    ...
    let mut queried_values_by_column_iter = queried_values_by_column.span().into_iter();

    loop {
        // Check there are the correct number of column values.
        if let Option::Some(column_queries) = queried_values_by_column_iter.next() {
...
fn accumulate_row_quotients(
    ...
    for batch_i in 0..n_batches {
        ...
        for sample_i in 0..batch_size {
            ...
            let column = *queried_values_by_column.at(*column_index);
...
mod tests {
    ...
    fn test_fri_answers_for_log_size() {
        ...
        let queried_values_by_column = array![
            @col0_query_values, @col1_query_values, @col2_query_values,
        ];

        let res = fri_answers_for_log_size(
            log_size, samples_by_column, random_coeff, query_positions, queried_values_by_column,
        )
            .unwrap();
    ...
    fn test_fri_answers() {
        ...
        let queried_values_by_column = array![col0_query_values, col1_query_values];
    ...
    fn test_accumulate_row_quotients() {
        ...
        let queried_values_by_column = array![@column0_query_values, @column1_query_values];
        let p0 = QM31_CIRCLE_GEN;
        let p1 = QM31_CIRCLE_GEN + QM31_CIRCLE_GEN;
        ...
        let res = accumulate_row_quotients(
            @sample_batches, @queried_values_by_column, @quotient_constants, row, domain.at(0),
        );
    ...
    fn test_fri_answers_with_1000_columns() {
        ...
        let mut queried_values_by_column = array![
            @col0_query_values, @col1_query_values, @col2_query_values,
        ];
        for _ in samples_per_column.len()..n_columns {
            samples_per_column.append(@col1_samples);
            queried_values_by_column.append(@col_query_values);
        };
