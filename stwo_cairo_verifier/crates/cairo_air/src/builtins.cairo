...
pub fn builtins_log_sizes(claim: @CairoClaim) -> TreeArray<Span<u32>> {
    ...
    if let Some(add_mod_builtin) = claim.add_mod_builtin {
        log_sizes.append(add_mod_builtin.log_sizes());
    }
    if let Some(bitwise_builtin) = claim.bitwise_builtin {
        log_sizes.append(bitwise_builtin.log_sizes());
    }
    if let Some(mul_mod_builtin) = claim.mul_mod_builtin {
        log_sizes.append(mul_mod_builtin.log_sizes());
    }
    if let Some(pedersen_builtin) = claim.pedersen_builtin {
        log_sizes.append(pedersen_builtin.log_sizes());
    }
    if let Some(poseidon_builtin) = claim.poseidon_builtin {
        log_sizes.append(poseidon_builtin.log_sizes());
    }
    if let Some(range_check96_builtin) = claim.range_check96_builtin {
        log_sizes.append(range_check96_builtin.log_sizes());
    }
    if let Some(range_check_builtin) = claim.range_check_builtin {
        log_sizes.append(range_check_builtin.log_sizes());
    }
