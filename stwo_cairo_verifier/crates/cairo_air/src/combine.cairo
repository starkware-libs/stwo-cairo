...
use bounded_int::{
    AddHelper, BoundedInt, DivRemHelper, MulHelper, NZ_U9_SHIFT, bounded_int_add, bounded_int_mul,
    div_rem, upcast,
};
...
fn add_to_sum_with_shift(ref sum: QM31, msb: u9, lsb: U23_BOUNDED_INT, shift: u9, alpha: QM31) {
    sum = sum * alpha
        + M31Trait::new(upcast::<_, M31InnerT>(bounded_int_add(bounded_int_mul(msb, shift), lsb)))
            .into();
}
