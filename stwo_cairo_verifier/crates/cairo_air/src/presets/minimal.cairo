...
const PREPROCESSED_COLUMNS: [PreprocessedColumn; 76] = [
...
fn preprocessed_root(
    ...
    match log_blowup_factor - 1 {
        0 => stwo_verifier_core::vcs::blake2s_hasher::Blake2sHash {
            hash: BoxImpl::new(
                [
                    2923431093, 137549136, 1055611113, 2672767554, 3002550851, 711980402,
                    3066614306, 1923556174,
                ],
            ),
        },
...
struct CairoInteractionElements {
    ...
    // We keep unused elements so that random coefficients are drawn deterministically.
...
pub struct BuiltinsClaim {
    pub add_mod_builtin: Option<()>,
    pub bitwise_builtin: Option<crate::components::bitwise_builtin::Claim>,
    pub mul_mod_builtin: Option<()>,
    pub pedersen_builtin: Option<()>,
    pub poseidon_builtin: Option<()>,
...
pub struct CairoClaim {
    ...
    // Using dummy optional claims to avoid deserialization errors (expecting None).
    pub pedersen_context: Option<()>,
    pub poseidon_context: Option<()>,
...
fn verify_builtins(builtins_claim: @BuiltinsClaim, segment_ranges: @PublicSegmentRanges) {
    ...
    if let Some(pedersen) = segment_ranges.pedersen {
        assert!(pedersen.start_ptr.value == pedersen.stop_ptr.value);
    }
    if let Some(poseidon) = segment_ranges.poseidon {
        assert!(poseidon.start_ptr.value == poseidon.stop_ptr.value);
    }
    if let Some(range_check_96) = segment_ranges.range_check_96 {
        assert!(range_check_96.start_ptr.value == range_check_96.stop_ptr.value);
    }
    if let Some(add_mod) = segment_ranges.add_mod {
        assert!(add_mod.start_ptr.value == add_mod.stop_ptr.value);
    }
    if let Some(mul_mod) = segment_ranges.mul_mod {
        assert!(mul_mod.start_ptr.value == mul_mod.stop_ptr.value);
    }

    // TODO: check other unused builtins
    // TODO: check other unused builtins
    // Output builtin.
