...
pub fn evaluate_constraints_at_point(
    ...
    // The component performs `n_column_wise_constraints` column-wise pairwise sums, followed by
    // a final constraint that performs a row-wise sum in addition to a column-wise pairwise sum.
    let n_column_wise_constraints = (MEMORY_ADDRESS_TO_ID_SPLIT / 2) - 1;

    // Perform the column-wise pairwise sums.
    for pair_index in 0..n_column_wise_constraints {
        // Get two (id, multiplicity) tuples from the trace.
    ...
    let [neg_1_cum_sum, curr_cum_sum] = columns_as_two_qm31(
