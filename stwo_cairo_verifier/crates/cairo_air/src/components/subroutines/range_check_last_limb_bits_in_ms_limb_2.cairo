...
pub fn range_check_last_limb_bits_in_ms_limb_2_evaluate(
    ...
    let constraint_quotient = ((msb_col0 * (qm31_const::<1, 0, 0, 0>() - msb_col0))) * domain_vanishing_eval_inv;
    sum = sum * random_coeff + constraint_quotient;let bit_before_msb_tmp_f851f_1: QM31 = (range_check_last_limb_bits_in_ms_limb_2_input - (msb_col0 * qm31_const::<2, 0, 0, 0>()));

    // Constraint - bit before msb is a bit
    let constraint_quotient = ((bit_before_msb_tmp_f851f_1 * (qm31_const::<1, 0, 0, 0>() - bit_before_msb_tmp_f851f_1))) * domain_vanishing_eval_inv;
