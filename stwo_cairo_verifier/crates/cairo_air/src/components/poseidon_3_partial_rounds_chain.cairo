// This file was created by the AIR team.

use crate::components::subroutines::poseidon_partial_round::poseidon_partial_round_evaluate;
use crate::prelude::*;

pub const N_TRACE_COLUMNS: usize = 169;
pub const N_INTERACTION_COLUMNS: usize = 36;
pub const RELATION_USES_PER_ROW: [(felt252, u32); 6] = [
    ('PoseidonRoundKeys', 1), ('Cube252', 3), ('RangeCheck_4_4_4_4', 6), ('RangeCheck_4_4', 3),
    ('RangeCheck252Width27', 3), ('Poseidon3PartialRoundsChain', 1),
];

#[derive(Drop, Serde, Copy)]
pub struct Claim {
    pub log_size: u32,
}

pub impl ClaimImpl of ClaimTrait<Claim> {
    fn log_sizes(self: @Claim) -> TreeArray<Span<u32>> {
        let log_size = *(self.log_size);
        let preprocessed_log_sizes = array![log_size].span();
        let trace_log_sizes = [log_size; N_TRACE_COLUMNS].span();
        let interaction_log_sizes = [log_size; N_INTERACTION_COLUMNS].span();
        array![preprocessed_log_sizes, trace_log_sizes, interaction_log_sizes]
    }

    fn mix_into(self: @Claim, ref channel: Channel) {
        channel.mix_u64((*(self.log_size)).into());
    }

    fn accumulate_relation_uses(self: @Claim, ref relation_uses: RelationUsesDict) {
        accumulate_relation_uses(ref relation_uses, RELATION_USES_PER_ROW.span(), *self.log_size);
    }
}


#[derive(Drop)]
pub struct Component {
    pub claim: Claim,
    pub claimed_sum: QM31,
    pub common_lookup_elements: CommonLookupElements,
}

pub impl NewComponentImpl of NewComponent<Component> {
    type Claim = Claim;

    fn new(
        claim: @Claim, claimed_sum: QM31, common_lookup_elements: @CommonLookupElements,
    ) -> Component {
        Component {
            claim: *claim, claimed_sum, common_lookup_elements: common_lookup_elements.clone(),
        }
    }
}

pub impl AirComponentImpl of AirComponent<Component> {
    fn evaluate_constraints_at_point(
        self: @Component,
        ref sum: QM31,
        ref preprocessed_mask_values: PreprocessedMaskValues,
        ref trace_mask_values: ColumnSpan<Span<QM31>>,
        ref interaction_trace_mask_values: ColumnSpan<Span<QM31>>,
        random_coeff: QM31,
        public_params: Span<u32>,
    ) {
        let log_size = *(self.claim.log_size);
        let claimed_sum = *self.claimed_sum;
        let column_size = m31(pow2(log_size));
        let mut poseidon_round_keys_sum_0: QM31 = Zero::zero();
        let mut numerator_0: QM31 = Zero::zero();
        let mut cube_252_sum_1: QM31 = Zero::zero();
        let mut numerator_1: QM31 = Zero::zero();
        let mut range_check_4_4_4_4_sum_2: QM31 = Zero::zero();
        let mut numerator_2: QM31 = Zero::zero();
        let mut range_check_4_4_4_4_sum_3: QM31 = Zero::zero();
        let mut numerator_3: QM31 = Zero::zero();
        let mut range_check_4_4_sum_4: QM31 = Zero::zero();
        let mut numerator_4: QM31 = Zero::zero();
        let mut range_check_252_width_27_sum_5: QM31 = Zero::zero();
        let mut numerator_5: QM31 = Zero::zero();
        let mut cube_252_sum_6: QM31 = Zero::zero();
        let mut numerator_6: QM31 = Zero::zero();
        let mut range_check_4_4_4_4_sum_7: QM31 = Zero::zero();
        let mut numerator_7: QM31 = Zero::zero();
        let mut range_check_4_4_4_4_sum_8: QM31 = Zero::zero();
        let mut numerator_8: QM31 = Zero::zero();
        let mut range_check_4_4_sum_9: QM31 = Zero::zero();
        let mut numerator_9: QM31 = Zero::zero();
        let mut range_check_252_width_27_sum_10: QM31 = Zero::zero();
        let mut numerator_10: QM31 = Zero::zero();
        let mut cube_252_sum_11: QM31 = Zero::zero();
        let mut numerator_11: QM31 = Zero::zero();
        let mut range_check_4_4_4_4_sum_12: QM31 = Zero::zero();
        let mut numerator_12: QM31 = Zero::zero();
        let mut range_check_4_4_4_4_sum_13: QM31 = Zero::zero();
        let mut numerator_13: QM31 = Zero::zero();
        let mut range_check_4_4_sum_14: QM31 = Zero::zero();
        let mut numerator_14: QM31 = Zero::zero();
        let mut range_check_252_width_27_sum_15: QM31 = Zero::zero();
        let mut numerator_15: QM31 = Zero::zero();
        let mut poseidon_3_partial_rounds_chain_sum_16: QM31 = Zero::zero();
        let mut numerator_16: QM31 = Zero::zero();
        let mut poseidon_3_partial_rounds_chain_sum_17: QM31 = Zero::zero();
        let mut numerator_17: QM31 = Zero::zero();

        let [
            enabler_col0,
            input_limb_0_col1,
            input_limb_1_col2,
            input_limb_2_col3,
            input_limb_3_col4,
            input_limb_4_col5,
            input_limb_5_col6,
            input_limb_6_col7,
            input_limb_7_col8,
            input_limb_8_col9,
            input_limb_9_col10,
            input_limb_10_col11,
            input_limb_11_col12,
            input_limb_12_col13,
            input_limb_13_col14,
            input_limb_14_col15,
            input_limb_15_col16,
            input_limb_16_col17,
            input_limb_17_col18,
            input_limb_18_col19,
            input_limb_19_col20,
            input_limb_20_col21,
            input_limb_21_col22,
            input_limb_22_col23,
            input_limb_23_col24,
            input_limb_24_col25,
            input_limb_25_col26,
            input_limb_26_col27,
            input_limb_27_col28,
            input_limb_28_col29,
            input_limb_29_col30,
            input_limb_30_col31,
            input_limb_31_col32,
            input_limb_32_col33,
            input_limb_33_col34,
            input_limb_34_col35,
            input_limb_35_col36,
            input_limb_36_col37,
            input_limb_37_col38,
            input_limb_38_col39,
            input_limb_39_col40,
            input_limb_40_col41,
            input_limb_41_col42,
            poseidon_round_keys_output_limb_0_col43,
            poseidon_round_keys_output_limb_1_col44,
            poseidon_round_keys_output_limb_2_col45,
            poseidon_round_keys_output_limb_3_col46,
            poseidon_round_keys_output_limb_4_col47,
            poseidon_round_keys_output_limb_5_col48,
            poseidon_round_keys_output_limb_6_col49,
            poseidon_round_keys_output_limb_7_col50,
            poseidon_round_keys_output_limb_8_col51,
            poseidon_round_keys_output_limb_9_col52,
            poseidon_round_keys_output_limb_10_col53,
            poseidon_round_keys_output_limb_11_col54,
            poseidon_round_keys_output_limb_12_col55,
            poseidon_round_keys_output_limb_13_col56,
            poseidon_round_keys_output_limb_14_col57,
            poseidon_round_keys_output_limb_15_col58,
            poseidon_round_keys_output_limb_16_col59,
            poseidon_round_keys_output_limb_17_col60,
            poseidon_round_keys_output_limb_18_col61,
            poseidon_round_keys_output_limb_19_col62,
            poseidon_round_keys_output_limb_20_col63,
            poseidon_round_keys_output_limb_21_col64,
            poseidon_round_keys_output_limb_22_col65,
            poseidon_round_keys_output_limb_23_col66,
            poseidon_round_keys_output_limb_24_col67,
            poseidon_round_keys_output_limb_25_col68,
            poseidon_round_keys_output_limb_26_col69,
            poseidon_round_keys_output_limb_27_col70,
            poseidon_round_keys_output_limb_28_col71,
            poseidon_round_keys_output_limb_29_col72,
            cube_252_output_limb_0_col73,
            cube_252_output_limb_1_col74,
            cube_252_output_limb_2_col75,
            cube_252_output_limb_3_col76,
            cube_252_output_limb_4_col77,
            cube_252_output_limb_5_col78,
            cube_252_output_limb_6_col79,
            cube_252_output_limb_7_col80,
            cube_252_output_limb_8_col81,
            cube_252_output_limb_9_col82,
            combination_limb_0_col83,
            combination_limb_1_col84,
            combination_limb_2_col85,
            combination_limb_3_col86,
            combination_limb_4_col87,
            combination_limb_5_col88,
            combination_limb_6_col89,
            combination_limb_7_col90,
            combination_limb_8_col91,
            combination_limb_9_col92,
            p_coef_col93,
            combination_limb_0_col94,
            combination_limb_1_col95,
            combination_limb_2_col96,
            combination_limb_3_col97,
            combination_limb_4_col98,
            combination_limb_5_col99,
            combination_limb_6_col100,
            combination_limb_7_col101,
            combination_limb_8_col102,
            combination_limb_9_col103,
            p_coef_col104,
            cube_252_output_limb_0_col105,
            cube_252_output_limb_1_col106,
            cube_252_output_limb_2_col107,
            cube_252_output_limb_3_col108,
            cube_252_output_limb_4_col109,
            cube_252_output_limb_5_col110,
            cube_252_output_limb_6_col111,
            cube_252_output_limb_7_col112,
            cube_252_output_limb_8_col113,
            cube_252_output_limb_9_col114,
            combination_limb_0_col115,
            combination_limb_1_col116,
            combination_limb_2_col117,
            combination_limb_3_col118,
            combination_limb_4_col119,
            combination_limb_5_col120,
            combination_limb_6_col121,
            combination_limb_7_col122,
            combination_limb_8_col123,
            combination_limb_9_col124,
            p_coef_col125,
            combination_limb_0_col126,
            combination_limb_1_col127,
            combination_limb_2_col128,
            combination_limb_3_col129,
            combination_limb_4_col130,
            combination_limb_5_col131,
            combination_limb_6_col132,
            combination_limb_7_col133,
            combination_limb_8_col134,
            combination_limb_9_col135,
            p_coef_col136,
            cube_252_output_limb_0_col137,
            cube_252_output_limb_1_col138,
            cube_252_output_limb_2_col139,
            cube_252_output_limb_3_col140,
            cube_252_output_limb_4_col141,
            cube_252_output_limb_5_col142,
            cube_252_output_limb_6_col143,
            cube_252_output_limb_7_col144,
            cube_252_output_limb_8_col145,
            cube_252_output_limb_9_col146,
            combination_limb_0_col147,
            combination_limb_1_col148,
            combination_limb_2_col149,
            combination_limb_3_col150,
            combination_limb_4_col151,
            combination_limb_5_col152,
            combination_limb_6_col153,
            combination_limb_7_col154,
            combination_limb_8_col155,
            combination_limb_9_col156,
            p_coef_col157,
            combination_limb_0_col158,
            combination_limb_1_col159,
            combination_limb_2_col160,
            combination_limb_3_col161,
            combination_limb_4_col162,
            combination_limb_5_col163,
            combination_limb_6_col164,
            combination_limb_7_col165,
            combination_limb_8_col166,
            combination_limb_9_col167,
            p_coef_col168,
        ]: [Span<QM31>; 169] =
            (*trace_mask_values
            .multi_pop_front()
            .unwrap())
            .unbox();
        let [enabler_col0]: [QM31; 1] = (*enabler_col0.try_into().unwrap()).unbox();
        let [input_limb_0_col1]: [QM31; 1] = (*input_limb_0_col1.try_into().unwrap()).unbox();
        let [input_limb_1_col2]: [QM31; 1] = (*input_limb_1_col2.try_into().unwrap()).unbox();
        let [input_limb_2_col3]: [QM31; 1] = (*input_limb_2_col3.try_into().unwrap()).unbox();
        let [input_limb_3_col4]: [QM31; 1] = (*input_limb_3_col4.try_into().unwrap()).unbox();
        let [input_limb_4_col5]: [QM31; 1] = (*input_limb_4_col5.try_into().unwrap()).unbox();
        let [input_limb_5_col6]: [QM31; 1] = (*input_limb_5_col6.try_into().unwrap()).unbox();
        let [input_limb_6_col7]: [QM31; 1] = (*input_limb_6_col7.try_into().unwrap()).unbox();
        let [input_limb_7_col8]: [QM31; 1] = (*input_limb_7_col8.try_into().unwrap()).unbox();
        let [input_limb_8_col9]: [QM31; 1] = (*input_limb_8_col9.try_into().unwrap()).unbox();
        let [input_limb_9_col10]: [QM31; 1] = (*input_limb_9_col10.try_into().unwrap()).unbox();
        let [input_limb_10_col11]: [QM31; 1] = (*input_limb_10_col11.try_into().unwrap()).unbox();
        let [input_limb_11_col12]: [QM31; 1] = (*input_limb_11_col12.try_into().unwrap()).unbox();
        let [input_limb_12_col13]: [QM31; 1] = (*input_limb_12_col13.try_into().unwrap()).unbox();
        let [input_limb_13_col14]: [QM31; 1] = (*input_limb_13_col14.try_into().unwrap()).unbox();
        let [input_limb_14_col15]: [QM31; 1] = (*input_limb_14_col15.try_into().unwrap()).unbox();
        let [input_limb_15_col16]: [QM31; 1] = (*input_limb_15_col16.try_into().unwrap()).unbox();
        let [input_limb_16_col17]: [QM31; 1] = (*input_limb_16_col17.try_into().unwrap()).unbox();
        let [input_limb_17_col18]: [QM31; 1] = (*input_limb_17_col18.try_into().unwrap()).unbox();
        let [input_limb_18_col19]: [QM31; 1] = (*input_limb_18_col19.try_into().unwrap()).unbox();
        let [input_limb_19_col20]: [QM31; 1] = (*input_limb_19_col20.try_into().unwrap()).unbox();
        let [input_limb_20_col21]: [QM31; 1] = (*input_limb_20_col21.try_into().unwrap()).unbox();
        let [input_limb_21_col22]: [QM31; 1] = (*input_limb_21_col22.try_into().unwrap()).unbox();
        let [input_limb_22_col23]: [QM31; 1] = (*input_limb_22_col23.try_into().unwrap()).unbox();
        let [input_limb_23_col24]: [QM31; 1] = (*input_limb_23_col24.try_into().unwrap()).unbox();
        let [input_limb_24_col25]: [QM31; 1] = (*input_limb_24_col25.try_into().unwrap()).unbox();
        let [input_limb_25_col26]: [QM31; 1] = (*input_limb_25_col26.try_into().unwrap()).unbox();
        let [input_limb_26_col27]: [QM31; 1] = (*input_limb_26_col27.try_into().unwrap()).unbox();
        let [input_limb_27_col28]: [QM31; 1] = (*input_limb_27_col28.try_into().unwrap()).unbox();
        let [input_limb_28_col29]: [QM31; 1] = (*input_limb_28_col29.try_into().unwrap()).unbox();
        let [input_limb_29_col30]: [QM31; 1] = (*input_limb_29_col30.try_into().unwrap()).unbox();
        let [input_limb_30_col31]: [QM31; 1] = (*input_limb_30_col31.try_into().unwrap()).unbox();
        let [input_limb_31_col32]: [QM31; 1] = (*input_limb_31_col32.try_into().unwrap()).unbox();
        let [input_limb_32_col33]: [QM31; 1] = (*input_limb_32_col33.try_into().unwrap()).unbox();
        let [input_limb_33_col34]: [QM31; 1] = (*input_limb_33_col34.try_into().unwrap()).unbox();
        let [input_limb_34_col35]: [QM31; 1] = (*input_limb_34_col35.try_into().unwrap()).unbox();
        let [input_limb_35_col36]: [QM31; 1] = (*input_limb_35_col36.try_into().unwrap()).unbox();
        let [input_limb_36_col37]: [QM31; 1] = (*input_limb_36_col37.try_into().unwrap()).unbox();
        let [input_limb_37_col38]: [QM31; 1] = (*input_limb_37_col38.try_into().unwrap()).unbox();
        let [input_limb_38_col39]: [QM31; 1] = (*input_limb_38_col39.try_into().unwrap()).unbox();
        let [input_limb_39_col40]: [QM31; 1] = (*input_limb_39_col40.try_into().unwrap()).unbox();
        let [input_limb_40_col41]: [QM31; 1] = (*input_limb_40_col41.try_into().unwrap()).unbox();
        let [input_limb_41_col42]: [QM31; 1] = (*input_limb_41_col42.try_into().unwrap()).unbox();
        let [poseidon_round_keys_output_limb_0_col43]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_0_col43
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_1_col44]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_1_col44
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_2_col45]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_2_col45
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_3_col46]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_3_col46
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_4_col47]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_4_col47
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_5_col48]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_5_col48
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_6_col49]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_6_col49
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_7_col50]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_7_col50
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_8_col51]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_8_col51
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_9_col52]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_9_col52
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_10_col53]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_10_col53
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_11_col54]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_11_col54
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_12_col55]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_12_col55
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_13_col56]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_13_col56
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_14_col57]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_14_col57
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_15_col58]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_15_col58
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_16_col59]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_16_col59
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_17_col60]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_17_col60
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_18_col61]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_18_col61
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_19_col62]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_19_col62
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_20_col63]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_20_col63
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_21_col64]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_21_col64
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_22_col65]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_22_col65
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_23_col66]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_23_col66
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_24_col67]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_24_col67
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_25_col68]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_25_col68
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_26_col69]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_26_col69
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_27_col70]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_27_col70
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_28_col71]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_28_col71
            .try_into()
            .unwrap())
            .unbox();
        let [poseidon_round_keys_output_limb_29_col72]: [QM31; 1] =
            (*poseidon_round_keys_output_limb_29_col72
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_0_col73]: [QM31; 1] = (*cube_252_output_limb_0_col73
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_1_col74]: [QM31; 1] = (*cube_252_output_limb_1_col74
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_2_col75]: [QM31; 1] = (*cube_252_output_limb_2_col75
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_3_col76]: [QM31; 1] = (*cube_252_output_limb_3_col76
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_4_col77]: [QM31; 1] = (*cube_252_output_limb_4_col77
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_5_col78]: [QM31; 1] = (*cube_252_output_limb_5_col78
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_6_col79]: [QM31; 1] = (*cube_252_output_limb_6_col79
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_7_col80]: [QM31; 1] = (*cube_252_output_limb_7_col80
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_8_col81]: [QM31; 1] = (*cube_252_output_limb_8_col81
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_9_col82]: [QM31; 1] = (*cube_252_output_limb_9_col82
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_0_col83]: [QM31; 1] = (*combination_limb_0_col83.try_into().unwrap())
            .unbox();
        let [combination_limb_1_col84]: [QM31; 1] = (*combination_limb_1_col84.try_into().unwrap())
            .unbox();
        let [combination_limb_2_col85]: [QM31; 1] = (*combination_limb_2_col85.try_into().unwrap())
            .unbox();
        let [combination_limb_3_col86]: [QM31; 1] = (*combination_limb_3_col86.try_into().unwrap())
            .unbox();
        let [combination_limb_4_col87]: [QM31; 1] = (*combination_limb_4_col87.try_into().unwrap())
            .unbox();
        let [combination_limb_5_col88]: [QM31; 1] = (*combination_limb_5_col88.try_into().unwrap())
            .unbox();
        let [combination_limb_6_col89]: [QM31; 1] = (*combination_limb_6_col89.try_into().unwrap())
            .unbox();
        let [combination_limb_7_col90]: [QM31; 1] = (*combination_limb_7_col90.try_into().unwrap())
            .unbox();
        let [combination_limb_8_col91]: [QM31; 1] = (*combination_limb_8_col91.try_into().unwrap())
            .unbox();
        let [combination_limb_9_col92]: [QM31; 1] = (*combination_limb_9_col92.try_into().unwrap())
            .unbox();
        let [p_coef_col93]: [QM31; 1] = (*p_coef_col93.try_into().unwrap()).unbox();
        let [combination_limb_0_col94]: [QM31; 1] = (*combination_limb_0_col94.try_into().unwrap())
            .unbox();
        let [combination_limb_1_col95]: [QM31; 1] = (*combination_limb_1_col95.try_into().unwrap())
            .unbox();
        let [combination_limb_2_col96]: [QM31; 1] = (*combination_limb_2_col96.try_into().unwrap())
            .unbox();
        let [combination_limb_3_col97]: [QM31; 1] = (*combination_limb_3_col97.try_into().unwrap())
            .unbox();
        let [combination_limb_4_col98]: [QM31; 1] = (*combination_limb_4_col98.try_into().unwrap())
            .unbox();
        let [combination_limb_5_col99]: [QM31; 1] = (*combination_limb_5_col99.try_into().unwrap())
            .unbox();
        let [combination_limb_6_col100]: [QM31; 1] = (*combination_limb_6_col100
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_7_col101]: [QM31; 1] = (*combination_limb_7_col101
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_8_col102]: [QM31; 1] = (*combination_limb_8_col102
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_9_col103]: [QM31; 1] = (*combination_limb_9_col103
            .try_into()
            .unwrap())
            .unbox();
        let [p_coef_col104]: [QM31; 1] = (*p_coef_col104.try_into().unwrap()).unbox();
        let [cube_252_output_limb_0_col105]: [QM31; 1] = (*cube_252_output_limb_0_col105
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_1_col106]: [QM31; 1] = (*cube_252_output_limb_1_col106
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_2_col107]: [QM31; 1] = (*cube_252_output_limb_2_col107
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_3_col108]: [QM31; 1] = (*cube_252_output_limb_3_col108
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_4_col109]: [QM31; 1] = (*cube_252_output_limb_4_col109
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_5_col110]: [QM31; 1] = (*cube_252_output_limb_5_col110
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_6_col111]: [QM31; 1] = (*cube_252_output_limb_6_col111
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_7_col112]: [QM31; 1] = (*cube_252_output_limb_7_col112
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_8_col113]: [QM31; 1] = (*cube_252_output_limb_8_col113
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_9_col114]: [QM31; 1] = (*cube_252_output_limb_9_col114
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_0_col115]: [QM31; 1] = (*combination_limb_0_col115
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_1_col116]: [QM31; 1] = (*combination_limb_1_col116
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_2_col117]: [QM31; 1] = (*combination_limb_2_col117
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_3_col118]: [QM31; 1] = (*combination_limb_3_col118
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_4_col119]: [QM31; 1] = (*combination_limb_4_col119
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_5_col120]: [QM31; 1] = (*combination_limb_5_col120
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_6_col121]: [QM31; 1] = (*combination_limb_6_col121
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_7_col122]: [QM31; 1] = (*combination_limb_7_col122
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_8_col123]: [QM31; 1] = (*combination_limb_8_col123
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_9_col124]: [QM31; 1] = (*combination_limb_9_col124
            .try_into()
            .unwrap())
            .unbox();
        let [p_coef_col125]: [QM31; 1] = (*p_coef_col125.try_into().unwrap()).unbox();
        let [combination_limb_0_col126]: [QM31; 1] = (*combination_limb_0_col126
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_1_col127]: [QM31; 1] = (*combination_limb_1_col127
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_2_col128]: [QM31; 1] = (*combination_limb_2_col128
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_3_col129]: [QM31; 1] = (*combination_limb_3_col129
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_4_col130]: [QM31; 1] = (*combination_limb_4_col130
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_5_col131]: [QM31; 1] = (*combination_limb_5_col131
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_6_col132]: [QM31; 1] = (*combination_limb_6_col132
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_7_col133]: [QM31; 1] = (*combination_limb_7_col133
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_8_col134]: [QM31; 1] = (*combination_limb_8_col134
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_9_col135]: [QM31; 1] = (*combination_limb_9_col135
            .try_into()
            .unwrap())
            .unbox();
        let [p_coef_col136]: [QM31; 1] = (*p_coef_col136.try_into().unwrap()).unbox();
        let [cube_252_output_limb_0_col137]: [QM31; 1] = (*cube_252_output_limb_0_col137
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_1_col138]: [QM31; 1] = (*cube_252_output_limb_1_col138
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_2_col139]: [QM31; 1] = (*cube_252_output_limb_2_col139
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_3_col140]: [QM31; 1] = (*cube_252_output_limb_3_col140
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_4_col141]: [QM31; 1] = (*cube_252_output_limb_4_col141
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_5_col142]: [QM31; 1] = (*cube_252_output_limb_5_col142
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_6_col143]: [QM31; 1] = (*cube_252_output_limb_6_col143
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_7_col144]: [QM31; 1] = (*cube_252_output_limb_7_col144
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_8_col145]: [QM31; 1] = (*cube_252_output_limb_8_col145
            .try_into()
            .unwrap())
            .unbox();
        let [cube_252_output_limb_9_col146]: [QM31; 1] = (*cube_252_output_limb_9_col146
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_0_col147]: [QM31; 1] = (*combination_limb_0_col147
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_1_col148]: [QM31; 1] = (*combination_limb_1_col148
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_2_col149]: [QM31; 1] = (*combination_limb_2_col149
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_3_col150]: [QM31; 1] = (*combination_limb_3_col150
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_4_col151]: [QM31; 1] = (*combination_limb_4_col151
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_5_col152]: [QM31; 1] = (*combination_limb_5_col152
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_6_col153]: [QM31; 1] = (*combination_limb_6_col153
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_7_col154]: [QM31; 1] = (*combination_limb_7_col154
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_8_col155]: [QM31; 1] = (*combination_limb_8_col155
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_9_col156]: [QM31; 1] = (*combination_limb_9_col156
            .try_into()
            .unwrap())
            .unbox();
        let [p_coef_col157]: [QM31; 1] = (*p_coef_col157.try_into().unwrap()).unbox();
        let [combination_limb_0_col158]: [QM31; 1] = (*combination_limb_0_col158
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_1_col159]: [QM31; 1] = (*combination_limb_1_col159
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_2_col160]: [QM31; 1] = (*combination_limb_2_col160
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_3_col161]: [QM31; 1] = (*combination_limb_3_col161
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_4_col162]: [QM31; 1] = (*combination_limb_4_col162
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_5_col163]: [QM31; 1] = (*combination_limb_5_col163
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_6_col164]: [QM31; 1] = (*combination_limb_6_col164
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_7_col165]: [QM31; 1] = (*combination_limb_7_col165
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_8_col166]: [QM31; 1] = (*combination_limb_8_col166
            .try_into()
            .unwrap())
            .unbox();
        let [combination_limb_9_col167]: [QM31; 1] = (*combination_limb_9_col167
            .try_into()
            .unwrap())
            .unbox();
        let [p_coef_col168]: [QM31; 1] = (*p_coef_col168.try_into().unwrap()).unbox();

        core::internal::revoke_ap_tracking();

        // Constraint - Enabler is a bit
        let constraint_quotient = (((enabler_col0 * enabler_col0) - enabler_col0));
        sum = sum * random_coeff + constraint_quotient;

        poseidon_round_keys_sum_0 = self
            .common_lookup_elements
            .combine_qm31(
                [
                    qm31_const::<1024310512, 0, 0, 0>(), input_limb_1_col2,
                    poseidon_round_keys_output_limb_0_col43,
                    poseidon_round_keys_output_limb_1_col44,
                    poseidon_round_keys_output_limb_2_col45,
                    poseidon_round_keys_output_limb_3_col46,
                    poseidon_round_keys_output_limb_4_col47,
                    poseidon_round_keys_output_limb_5_col48,
                    poseidon_round_keys_output_limb_6_col49,
                    poseidon_round_keys_output_limb_7_col50,
                    poseidon_round_keys_output_limb_8_col51,
                    poseidon_round_keys_output_limb_9_col52,
                    poseidon_round_keys_output_limb_10_col53,
                    poseidon_round_keys_output_limb_11_col54,
                    poseidon_round_keys_output_limb_12_col55,
                    poseidon_round_keys_output_limb_13_col56,
                    poseidon_round_keys_output_limb_14_col57,
                    poseidon_round_keys_output_limb_15_col58,
                    poseidon_round_keys_output_limb_16_col59,
                    poseidon_round_keys_output_limb_17_col60,
                    poseidon_round_keys_output_limb_18_col61,
                    poseidon_round_keys_output_limb_19_col62,
                    poseidon_round_keys_output_limb_20_col63,
                    poseidon_round_keys_output_limb_21_col64,
                    poseidon_round_keys_output_limb_22_col65,
                    poseidon_round_keys_output_limb_23_col66,
                    poseidon_round_keys_output_limb_24_col67,
                    poseidon_round_keys_output_limb_25_col68,
                    poseidon_round_keys_output_limb_26_col69,
                    poseidon_round_keys_output_limb_27_col70,
                    poseidon_round_keys_output_limb_28_col71,
                    poseidon_round_keys_output_limb_29_col72,
                ]
                    .span(),
            );
        numerator_0 = enabler_col0;
        poseidon_partial_round_evaluate(
            [
                input_limb_2_col3, input_limb_3_col4, input_limb_4_col5, input_limb_5_col6,
                input_limb_6_col7, input_limb_7_col8, input_limb_8_col9, input_limb_9_col10,
                input_limb_10_col11, input_limb_11_col12, input_limb_12_col13, input_limb_13_col14,
                input_limb_14_col15, input_limb_15_col16, input_limb_16_col17, input_limb_17_col18,
                input_limb_18_col19, input_limb_19_col20, input_limb_20_col21, input_limb_21_col22,
                input_limb_22_col23, input_limb_23_col24, input_limb_24_col25, input_limb_25_col26,
                input_limb_26_col27, input_limb_27_col28, input_limb_28_col29, input_limb_29_col30,
                input_limb_30_col31, input_limb_31_col32, input_limb_32_col33, input_limb_33_col34,
                input_limb_34_col35, input_limb_35_col36, input_limb_36_col37, input_limb_37_col38,
                input_limb_38_col39, input_limb_39_col40, input_limb_40_col41, input_limb_41_col42,
                poseidon_round_keys_output_limb_0_col43, poseidon_round_keys_output_limb_1_col44,
                poseidon_round_keys_output_limb_2_col45, poseidon_round_keys_output_limb_3_col46,
                poseidon_round_keys_output_limb_4_col47, poseidon_round_keys_output_limb_5_col48,
                poseidon_round_keys_output_limb_6_col49, poseidon_round_keys_output_limb_7_col50,
                poseidon_round_keys_output_limb_8_col51, poseidon_round_keys_output_limb_9_col52,
            ],
            enabler_col0,
            cube_252_output_limb_0_col73,
            cube_252_output_limb_1_col74,
            cube_252_output_limb_2_col75,
            cube_252_output_limb_3_col76,
            cube_252_output_limb_4_col77,
            cube_252_output_limb_5_col78,
            cube_252_output_limb_6_col79,
            cube_252_output_limb_7_col80,
            cube_252_output_limb_8_col81,
            cube_252_output_limb_9_col82,
            combination_limb_0_col83,
            combination_limb_1_col84,
            combination_limb_2_col85,
            combination_limb_3_col86,
            combination_limb_4_col87,
            combination_limb_5_col88,
            combination_limb_6_col89,
            combination_limb_7_col90,
            combination_limb_8_col91,
            combination_limb_9_col92,
            p_coef_col93,
            combination_limb_0_col94,
            combination_limb_1_col95,
            combination_limb_2_col96,
            combination_limb_3_col97,
            combination_limb_4_col98,
            combination_limb_5_col99,
            combination_limb_6_col100,
            combination_limb_7_col101,
            combination_limb_8_col102,
            combination_limb_9_col103,
            p_coef_col104,
            self.common_lookup_elements,
            ref cube_252_sum_1,
            ref numerator_1,
            ref range_check_4_4_4_4_sum_2,
            ref numerator_2,
            ref range_check_4_4_4_4_sum_3,
            ref numerator_3,
            ref range_check_4_4_sum_4,
            ref numerator_4,
            ref range_check_252_width_27_sum_5,
            ref numerator_5,
            ref sum,
            random_coeff,
        );
        poseidon_partial_round_evaluate(
            [
                input_limb_22_col23, input_limb_23_col24, input_limb_24_col25, input_limb_25_col26,
                input_limb_26_col27, input_limb_27_col28, input_limb_28_col29, input_limb_29_col30,
                input_limb_30_col31, input_limb_31_col32, input_limb_32_col33, input_limb_33_col34,
                input_limb_34_col35, input_limb_35_col36, input_limb_36_col37, input_limb_37_col38,
                input_limb_38_col39, input_limb_39_col40, input_limb_40_col41, input_limb_41_col42,
                cube_252_output_limb_0_col73, cube_252_output_limb_1_col74,
                cube_252_output_limb_2_col75, cube_252_output_limb_3_col76,
                cube_252_output_limb_4_col77, cube_252_output_limb_5_col78,
                cube_252_output_limb_6_col79, cube_252_output_limb_7_col80,
                cube_252_output_limb_8_col81, cube_252_output_limb_9_col82,
                combination_limb_0_col94, combination_limb_1_col95, combination_limb_2_col96,
                combination_limb_3_col97, combination_limb_4_col98, combination_limb_5_col99,
                combination_limb_6_col100, combination_limb_7_col101, combination_limb_8_col102,
                combination_limb_9_col103, poseidon_round_keys_output_limb_10_col53,
                poseidon_round_keys_output_limb_11_col54, poseidon_round_keys_output_limb_12_col55,
                poseidon_round_keys_output_limb_13_col56, poseidon_round_keys_output_limb_14_col57,
                poseidon_round_keys_output_limb_15_col58, poseidon_round_keys_output_limb_16_col59,
                poseidon_round_keys_output_limb_17_col60, poseidon_round_keys_output_limb_18_col61,
                poseidon_round_keys_output_limb_19_col62,
            ],
            enabler_col0,
            cube_252_output_limb_0_col105,
            cube_252_output_limb_1_col106,
            cube_252_output_limb_2_col107,
            cube_252_output_limb_3_col108,
            cube_252_output_limb_4_col109,
            cube_252_output_limb_5_col110,
            cube_252_output_limb_6_col111,
            cube_252_output_limb_7_col112,
            cube_252_output_limb_8_col113,
            cube_252_output_limb_9_col114,
            combination_limb_0_col115,
            combination_limb_1_col116,
            combination_limb_2_col117,
            combination_limb_3_col118,
            combination_limb_4_col119,
            combination_limb_5_col120,
            combination_limb_6_col121,
            combination_limb_7_col122,
            combination_limb_8_col123,
            combination_limb_9_col124,
            p_coef_col125,
            combination_limb_0_col126,
            combination_limb_1_col127,
            combination_limb_2_col128,
            combination_limb_3_col129,
            combination_limb_4_col130,
            combination_limb_5_col131,
            combination_limb_6_col132,
            combination_limb_7_col133,
            combination_limb_8_col134,
            combination_limb_9_col135,
            p_coef_col136,
            self.common_lookup_elements,
            ref cube_252_sum_6,
            ref numerator_6,
            ref range_check_4_4_4_4_sum_7,
            ref numerator_7,
            ref range_check_4_4_4_4_sum_8,
            ref numerator_8,
            ref range_check_4_4_sum_9,
            ref numerator_9,
            ref range_check_252_width_27_sum_10,
            ref numerator_10,
            ref sum,
            random_coeff,
        );
        poseidon_partial_round_evaluate(
            [
                cube_252_output_limb_0_col73, cube_252_output_limb_1_col74,
                cube_252_output_limb_2_col75, cube_252_output_limb_3_col76,
                cube_252_output_limb_4_col77, cube_252_output_limb_5_col78,
                cube_252_output_limb_6_col79, cube_252_output_limb_7_col80,
                cube_252_output_limb_8_col81, cube_252_output_limb_9_col82,
                combination_limb_0_col94, combination_limb_1_col95, combination_limb_2_col96,
                combination_limb_3_col97, combination_limb_4_col98, combination_limb_5_col99,
                combination_limb_6_col100, combination_limb_7_col101, combination_limb_8_col102,
                combination_limb_9_col103, cube_252_output_limb_0_col105,
                cube_252_output_limb_1_col106, cube_252_output_limb_2_col107,
                cube_252_output_limb_3_col108, cube_252_output_limb_4_col109,
                cube_252_output_limb_5_col110, cube_252_output_limb_6_col111,
                cube_252_output_limb_7_col112, cube_252_output_limb_8_col113,
                cube_252_output_limb_9_col114, combination_limb_0_col126, combination_limb_1_col127,
                combination_limb_2_col128, combination_limb_3_col129, combination_limb_4_col130,
                combination_limb_5_col131, combination_limb_6_col132, combination_limb_7_col133,
                combination_limb_8_col134, combination_limb_9_col135,
                poseidon_round_keys_output_limb_20_col63, poseidon_round_keys_output_limb_21_col64,
                poseidon_round_keys_output_limb_22_col65, poseidon_round_keys_output_limb_23_col66,
                poseidon_round_keys_output_limb_24_col67, poseidon_round_keys_output_limb_25_col68,
                poseidon_round_keys_output_limb_26_col69, poseidon_round_keys_output_limb_27_col70,
                poseidon_round_keys_output_limb_28_col71, poseidon_round_keys_output_limb_29_col72,
            ],
            enabler_col0,
            cube_252_output_limb_0_col137,
            cube_252_output_limb_1_col138,
            cube_252_output_limb_2_col139,
            cube_252_output_limb_3_col140,
            cube_252_output_limb_4_col141,
            cube_252_output_limb_5_col142,
            cube_252_output_limb_6_col143,
            cube_252_output_limb_7_col144,
            cube_252_output_limb_8_col145,
            cube_252_output_limb_9_col146,
            combination_limb_0_col147,
            combination_limb_1_col148,
            combination_limb_2_col149,
            combination_limb_3_col150,
            combination_limb_4_col151,
            combination_limb_5_col152,
            combination_limb_6_col153,
            combination_limb_7_col154,
            combination_limb_8_col155,
            combination_limb_9_col156,
            p_coef_col157,
            combination_limb_0_col158,
            combination_limb_1_col159,
            combination_limb_2_col160,
            combination_limb_3_col161,
            combination_limb_4_col162,
            combination_limb_5_col163,
            combination_limb_6_col164,
            combination_limb_7_col165,
            combination_limb_8_col166,
            combination_limb_9_col167,
            p_coef_col168,
            self.common_lookup_elements,
            ref cube_252_sum_11,
            ref numerator_11,
            ref range_check_4_4_4_4_sum_12,
            ref numerator_12,
            ref range_check_4_4_4_4_sum_13,
            ref numerator_13,
            ref range_check_4_4_sum_14,
            ref numerator_14,
            ref range_check_252_width_27_sum_15,
            ref numerator_15,
            ref sum,
            random_coeff,
        );

        poseidon_3_partial_rounds_chain_sum_16 = self
            .common_lookup_elements
            .combine_qm31(
                [
                    qm31_const::<1343313504, 0, 0, 0>(), input_limb_0_col1, input_limb_1_col2,
                    input_limb_2_col3, input_limb_3_col4, input_limb_4_col5, input_limb_5_col6,
                    input_limb_6_col7, input_limb_7_col8, input_limb_8_col9, input_limb_9_col10,
                    input_limb_10_col11, input_limb_11_col12, input_limb_12_col13,
                    input_limb_13_col14, input_limb_14_col15, input_limb_15_col16,
                    input_limb_16_col17, input_limb_17_col18, input_limb_18_col19,
                    input_limb_19_col20, input_limb_20_col21, input_limb_21_col22,
                    input_limb_22_col23, input_limb_23_col24, input_limb_24_col25,
                    input_limb_25_col26, input_limb_26_col27, input_limb_27_col28,
                    input_limb_28_col29, input_limb_29_col30, input_limb_30_col31,
                    input_limb_31_col32, input_limb_32_col33, input_limb_33_col34,
                    input_limb_34_col35, input_limb_35_col36, input_limb_36_col37,
                    input_limb_37_col38, input_limb_38_col39, input_limb_39_col40,
                    input_limb_40_col41, input_limb_41_col42,
                ]
                    .span(),
            );
        numerator_16 = enabler_col0;

        poseidon_3_partial_rounds_chain_sum_17 = self
            .common_lookup_elements
            .combine_qm31(
                [
                    qm31_const::<1343313504, 0, 0, 0>(), input_limb_0_col1,
                    (input_limb_1_col2 + qm31_const::<1, 0, 0, 0>()), cube_252_output_limb_0_col105,
                    cube_252_output_limb_1_col106, cube_252_output_limb_2_col107,
                    cube_252_output_limb_3_col108, cube_252_output_limb_4_col109,
                    cube_252_output_limb_5_col110, cube_252_output_limb_6_col111,
                    cube_252_output_limb_7_col112, cube_252_output_limb_8_col113,
                    cube_252_output_limb_9_col114, combination_limb_0_col126,
                    combination_limb_1_col127, combination_limb_2_col128, combination_limb_3_col129,
                    combination_limb_4_col130, combination_limb_5_col131, combination_limb_6_col132,
                    combination_limb_7_col133, combination_limb_8_col134, combination_limb_9_col135,
                    cube_252_output_limb_0_col137, cube_252_output_limb_1_col138,
                    cube_252_output_limb_2_col139, cube_252_output_limb_3_col140,
                    cube_252_output_limb_4_col141, cube_252_output_limb_5_col142,
                    cube_252_output_limb_6_col143, cube_252_output_limb_7_col144,
                    cube_252_output_limb_8_col145, cube_252_output_limb_9_col146,
                    combination_limb_0_col158, combination_limb_1_col159, combination_limb_2_col160,
                    combination_limb_3_col161, combination_limb_4_col162, combination_limb_5_col163,
                    combination_limb_6_col164, combination_limb_7_col165, combination_limb_8_col166,
                    combination_limb_9_col167,
                ]
                    .span(),
            );
        numerator_17 = enabler_col0;

        lookup_constraints(
            ref sum,
            random_coeff,
            claimed_sum,
            numerator_0,
            numerator_1,
            numerator_2,
            numerator_3,
            numerator_4,
            numerator_5,
            numerator_6,
            numerator_7,
            numerator_8,
            numerator_9,
            numerator_10,
            numerator_11,
            numerator_12,
            numerator_13,
            numerator_14,
            numerator_15,
            numerator_16,
            numerator_17,
            column_size,
            ref interaction_trace_mask_values,
            poseidon_round_keys_sum_0,
            cube_252_sum_1,
            range_check_4_4_4_4_sum_2,
            range_check_4_4_4_4_sum_3,
            range_check_4_4_sum_4,
            range_check_252_width_27_sum_5,
            cube_252_sum_6,
            range_check_4_4_4_4_sum_7,
            range_check_4_4_4_4_sum_8,
            range_check_4_4_sum_9,
            range_check_252_width_27_sum_10,
            cube_252_sum_11,
            range_check_4_4_4_4_sum_12,
            range_check_4_4_4_4_sum_13,
            range_check_4_4_sum_14,
            range_check_252_width_27_sum_15,
            poseidon_3_partial_rounds_chain_sum_16,
            poseidon_3_partial_rounds_chain_sum_17,
        );
    }
}


fn lookup_constraints(
    ref sum: QM31,
    random_coeff: QM31,
    claimed_sum: QM31,
    numerator_0: QM31,
    numerator_1: QM31,
    numerator_2: QM31,
    numerator_3: QM31,
    numerator_4: QM31,
    numerator_5: QM31,
    numerator_6: QM31,
    numerator_7: QM31,
    numerator_8: QM31,
    numerator_9: QM31,
    numerator_10: QM31,
    numerator_11: QM31,
    numerator_12: QM31,
    numerator_13: QM31,
    numerator_14: QM31,
    numerator_15: QM31,
    numerator_16: QM31,
    numerator_17: QM31,
    column_size: M31,
    ref interaction_trace_mask_values: ColumnSpan<Span<QM31>>,
    poseidon_round_keys_sum_0: QM31,
    cube_252_sum_1: QM31,
    range_check_4_4_4_4_sum_2: QM31,
    range_check_4_4_4_4_sum_3: QM31,
    range_check_4_4_sum_4: QM31,
    range_check_252_width_27_sum_5: QM31,
    cube_252_sum_6: QM31,
    range_check_4_4_4_4_sum_7: QM31,
    range_check_4_4_4_4_sum_8: QM31,
    range_check_4_4_sum_9: QM31,
    range_check_252_width_27_sum_10: QM31,
    cube_252_sum_11: QM31,
    range_check_4_4_4_4_sum_12: QM31,
    range_check_4_4_4_4_sum_13: QM31,
    range_check_4_4_sum_14: QM31,
    range_check_252_width_27_sum_15: QM31,
    poseidon_3_partial_rounds_chain_sum_16: QM31,
    poseidon_3_partial_rounds_chain_sum_17: QM31,
) {
    let [
        trace_2_col0,
        trace_2_col1,
        trace_2_col2,
        trace_2_col3,
        trace_2_col4,
        trace_2_col5,
        trace_2_col6,
        trace_2_col7,
        trace_2_col8,
        trace_2_col9,
        trace_2_col10,
        trace_2_col11,
        trace_2_col12,
        trace_2_col13,
        trace_2_col14,
        trace_2_col15,
        trace_2_col16,
        trace_2_col17,
        trace_2_col18,
        trace_2_col19,
        trace_2_col20,
        trace_2_col21,
        trace_2_col22,
        trace_2_col23,
        trace_2_col24,
        trace_2_col25,
        trace_2_col26,
        trace_2_col27,
        trace_2_col28,
        trace_2_col29,
        trace_2_col30,
        trace_2_col31,
        trace_2_col32,
        trace_2_col33,
        trace_2_col34,
        trace_2_col35,
    ]: [Span<QM31>; 36] =
        (*interaction_trace_mask_values
        .multi_pop_front()
        .unwrap())
        .unbox();

    let [trace_2_col0]: [QM31; 1] = (*trace_2_col0.try_into().unwrap()).unbox();
    let [trace_2_col1]: [QM31; 1] = (*trace_2_col1.try_into().unwrap()).unbox();
    let [trace_2_col2]: [QM31; 1] = (*trace_2_col2.try_into().unwrap()).unbox();
    let [trace_2_col3]: [QM31; 1] = (*trace_2_col3.try_into().unwrap()).unbox();
    let [trace_2_col4]: [QM31; 1] = (*trace_2_col4.try_into().unwrap()).unbox();
    let [trace_2_col5]: [QM31; 1] = (*trace_2_col5.try_into().unwrap()).unbox();
    let [trace_2_col6]: [QM31; 1] = (*trace_2_col6.try_into().unwrap()).unbox();
    let [trace_2_col7]: [QM31; 1] = (*trace_2_col7.try_into().unwrap()).unbox();
    let [trace_2_col8]: [QM31; 1] = (*trace_2_col8.try_into().unwrap()).unbox();
    let [trace_2_col9]: [QM31; 1] = (*trace_2_col9.try_into().unwrap()).unbox();
    let [trace_2_col10]: [QM31; 1] = (*trace_2_col10.try_into().unwrap()).unbox();
    let [trace_2_col11]: [QM31; 1] = (*trace_2_col11.try_into().unwrap()).unbox();
    let [trace_2_col12]: [QM31; 1] = (*trace_2_col12.try_into().unwrap()).unbox();
    let [trace_2_col13]: [QM31; 1] = (*trace_2_col13.try_into().unwrap()).unbox();
    let [trace_2_col14]: [QM31; 1] = (*trace_2_col14.try_into().unwrap()).unbox();
    let [trace_2_col15]: [QM31; 1] = (*trace_2_col15.try_into().unwrap()).unbox();
    let [trace_2_col16]: [QM31; 1] = (*trace_2_col16.try_into().unwrap()).unbox();
    let [trace_2_col17]: [QM31; 1] = (*trace_2_col17.try_into().unwrap()).unbox();
    let [trace_2_col18]: [QM31; 1] = (*trace_2_col18.try_into().unwrap()).unbox();
    let [trace_2_col19]: [QM31; 1] = (*trace_2_col19.try_into().unwrap()).unbox();
    let [trace_2_col20]: [QM31; 1] = (*trace_2_col20.try_into().unwrap()).unbox();
    let [trace_2_col21]: [QM31; 1] = (*trace_2_col21.try_into().unwrap()).unbox();
    let [trace_2_col22]: [QM31; 1] = (*trace_2_col22.try_into().unwrap()).unbox();
    let [trace_2_col23]: [QM31; 1] = (*trace_2_col23.try_into().unwrap()).unbox();
    let [trace_2_col24]: [QM31; 1] = (*trace_2_col24.try_into().unwrap()).unbox();
    let [trace_2_col25]: [QM31; 1] = (*trace_2_col25.try_into().unwrap()).unbox();
    let [trace_2_col26]: [QM31; 1] = (*trace_2_col26.try_into().unwrap()).unbox();
    let [trace_2_col27]: [QM31; 1] = (*trace_2_col27.try_into().unwrap()).unbox();
    let [trace_2_col28]: [QM31; 1] = (*trace_2_col28.try_into().unwrap()).unbox();
    let [trace_2_col29]: [QM31; 1] = (*trace_2_col29.try_into().unwrap()).unbox();
    let [trace_2_col30]: [QM31; 1] = (*trace_2_col30.try_into().unwrap()).unbox();
    let [trace_2_col31]: [QM31; 1] = (*trace_2_col31.try_into().unwrap()).unbox();
    let [trace_2_col32_neg1, trace_2_col32]: [QM31; 2] = (*trace_2_col32.try_into().unwrap())
        .unbox();
    let [trace_2_col33_neg1, trace_2_col33]: [QM31; 2] = (*trace_2_col33.try_into().unwrap())
        .unbox();
    let [trace_2_col34_neg1, trace_2_col34]: [QM31; 2] = (*trace_2_col34.try_into().unwrap())
        .unbox();
    let [trace_2_col35_neg1, trace_2_col35]: [QM31; 2] = (*trace_2_col35.try_into().unwrap())
        .unbox();

    core::internal::revoke_ap_tracking();

    let constraint_quotient = (((QM31Impl::from_partial_evals(
        [trace_2_col0, trace_2_col1, trace_2_col2, trace_2_col3],
    ))
        * poseidon_round_keys_sum_0
        * cube_252_sum_1)
        - (poseidon_round_keys_sum_0 * numerator_1)
        - (cube_252_sum_1 * numerator_0));
    sum = sum * random_coeff + constraint_quotient;

    let constraint_quotient = (((QM31Impl::from_partial_evals(
        [trace_2_col4, trace_2_col5, trace_2_col6, trace_2_col7],
    )
        - QM31Impl::from_partial_evals([trace_2_col0, trace_2_col1, trace_2_col2, trace_2_col3]))
        * range_check_4_4_4_4_sum_2
        * range_check_4_4_4_4_sum_3)
        - (range_check_4_4_4_4_sum_2 * numerator_3)
        - (range_check_4_4_4_4_sum_3 * numerator_2));
    sum = sum * random_coeff + constraint_quotient;

    let constraint_quotient = (((QM31Impl::from_partial_evals(
        [trace_2_col8, trace_2_col9, trace_2_col10, trace_2_col11],
    )
        - QM31Impl::from_partial_evals([trace_2_col4, trace_2_col5, trace_2_col6, trace_2_col7]))
        * range_check_4_4_sum_4
        * range_check_252_width_27_sum_5)
        - (range_check_4_4_sum_4 * numerator_5)
        - (range_check_252_width_27_sum_5 * numerator_4));
    sum = sum * random_coeff + constraint_quotient;

    let constraint_quotient = (((QM31Impl::from_partial_evals(
        [trace_2_col12, trace_2_col13, trace_2_col14, trace_2_col15],
    )
        - QM31Impl::from_partial_evals([trace_2_col8, trace_2_col9, trace_2_col10, trace_2_col11]))
        * cube_252_sum_6
        * range_check_4_4_4_4_sum_7)
        - (cube_252_sum_6 * numerator_7)
        - (range_check_4_4_4_4_sum_7 * numerator_6));
    sum = sum * random_coeff + constraint_quotient;

    let constraint_quotient = (((QM31Impl::from_partial_evals(
        [trace_2_col16, trace_2_col17, trace_2_col18, trace_2_col19],
    )
        - QM31Impl::from_partial_evals(
            [trace_2_col12, trace_2_col13, trace_2_col14, trace_2_col15],
        ))
        * range_check_4_4_4_4_sum_8
        * range_check_4_4_sum_9)
        - (range_check_4_4_4_4_sum_8 * numerator_9)
        - (range_check_4_4_sum_9 * numerator_8));
    sum = sum * random_coeff + constraint_quotient;

    let constraint_quotient = (((QM31Impl::from_partial_evals(
        [trace_2_col20, trace_2_col21, trace_2_col22, trace_2_col23],
    )
        - QM31Impl::from_partial_evals(
            [trace_2_col16, trace_2_col17, trace_2_col18, trace_2_col19],
        ))
        * range_check_252_width_27_sum_10
        * cube_252_sum_11)
        - (range_check_252_width_27_sum_10 * numerator_11)
        - (cube_252_sum_11 * numerator_10));
    sum = sum * random_coeff + constraint_quotient;

    let constraint_quotient = (((QM31Impl::from_partial_evals(
        [trace_2_col24, trace_2_col25, trace_2_col26, trace_2_col27],
    )
        - QM31Impl::from_partial_evals(
            [trace_2_col20, trace_2_col21, trace_2_col22, trace_2_col23],
        ))
        * range_check_4_4_4_4_sum_12
        * range_check_4_4_4_4_sum_13)
        - (range_check_4_4_4_4_sum_12 * numerator_13)
        - (range_check_4_4_4_4_sum_13 * numerator_12));
    sum = sum * random_coeff + constraint_quotient;

    let constraint_quotient = (((QM31Impl::from_partial_evals(
        [trace_2_col28, trace_2_col29, trace_2_col30, trace_2_col31],
    )
        - QM31Impl::from_partial_evals(
            [trace_2_col24, trace_2_col25, trace_2_col26, trace_2_col27],
        ))
        * range_check_4_4_sum_14
        * range_check_252_width_27_sum_15)
        - (range_check_4_4_sum_14 * numerator_15)
        - (range_check_252_width_27_sum_15 * numerator_14));
    sum = sum * random_coeff + constraint_quotient;

    let constraint_quotient = (((QM31Impl::from_partial_evals(
        [trace_2_col32, trace_2_col33, trace_2_col34, trace_2_col35],
    )
        - QM31Impl::from_partial_evals([trace_2_col28, trace_2_col29, trace_2_col30, trace_2_col31])
        - QM31Impl::from_partial_evals(
            [trace_2_col32_neg1, trace_2_col33_neg1, trace_2_col34_neg1, trace_2_col35_neg1],
        )
        + (claimed_sum * (column_size.inverse().into())))
        * poseidon_3_partial_rounds_chain_sum_16
        * poseidon_3_partial_rounds_chain_sum_17)
        + (poseidon_3_partial_rounds_chain_sum_16 * numerator_17)
        - (poseidon_3_partial_rounds_chain_sum_17 * numerator_16));
    sum = sum * random_coeff + constraint_quotient;
}
#[cfg(and(test, feature: "qm31_opcode"))]
mod tests {
    use core::array::ArrayImpl;
    use core::num::traits::Zero;
    use stwo_constraint_framework::AirComponent;
    #[allow(unused_imports)]
    use stwo_constraint_framework::test_utils::{make_interaction_trace, preprocessed_mask_add};
    #[allow(unused_imports)]
    use stwo_constraint_framework::{
        LookupElementsTrait, PreprocessedMaskValues, PreprocessedMaskValuesTrait,
    };
    use stwo_verifier_core::fields::qm31::{QM31, QM31Impl, QM31Trait, qm31_const};
    use crate::components::sample_evaluations::*;
    #[allow(unused_imports)]
    use crate::preprocessed_columns::*;
    use super::{Claim, Component};

    #[test]
    fn test_evaluation_result() {
        let component = Component {
            claim: Claim { log_size: 15 },
            claimed_sum: qm31_const::<1398335417, 314974026, 1722107152, 821933968>(),
            common_lookup_elements: LookupElementsTrait::from_z_alpha(
                qm31_const::<445623802, 202571636, 1360224996, 131355117>(),
                qm31_const::<476823935, 939223384, 62486082, 122423602>(),
            ),
        };
        let public_params = [].span();
        let mut sum: QM31 = Zero::zero();

        let mut preprocessed_trace = PreprocessedMaskValues { values: Default::default() };

        let mut trace_columns = [
            [qm31_const::<1659099300, 905558730, 651199673, 1375009625>()].span(),
            [qm31_const::<1591990121, 771341002, 584090809, 1375009625>()].span(),
            [qm31_const::<1793317658, 1173994186, 785417401, 1375009625>()].span(),
            [qm31_const::<1726208479, 1039776458, 718308537, 1375009625>()].span(),
            [qm31_const::<1390662584, 368687818, 382764217, 1375009625>()].span(),
            [qm31_const::<1323553405, 234470090, 315655353, 1375009625>()].span(),
            [qm31_const::<1524880942, 637123274, 516981945, 1375009625>()].span(),
            [qm31_const::<1457771763, 502905546, 449873081, 1375009625>()].span(),
            [qm31_const::<48489085, 1979300555, 1188070585, 1375009625>()].span(),
            [qm31_const::<2128863553, 1845082826, 1120961721, 1375009625>()].span(),
            [qm31_const::<1852335767, 645078115, 2059236183, 343880121>()].span(),
            [qm31_const::<1919444946, 779295843, 2126345047, 343880121>()].span(),
            [qm31_const::<1986554125, 913513571, 45970264, 343880122>()].span(),
            [qm31_const::<2053663304, 1047731299, 113079128, 343880122>()].span(),
            [qm31_const::<1583899051, 108207203, 1790800727, 343880121>()].span(),
            [qm31_const::<1651008230, 242424931, 1857909591, 343880121>()].span(),
            [qm31_const::<1718117409, 376642659, 1925018455, 343880121>()].span(),
            [qm31_const::<1785226588, 510860387, 1992127319, 343880121>()].span(),
            [qm31_const::<1315462335, 1718819938, 1522365270, 343880121>()].span(),
            [qm31_const::<1382571514, 1853037666, 1589474134, 343880121>()].span(),
            [qm31_const::<1986820986, 913513739, 45970432, 343880178>()].span(),
            [qm31_const::<1919711807, 779296011, 2126345215, 343880177>()].span(),
            [qm31_const::<2121039344, 1181949195, 180188160, 343880178>()].span(),
            [qm31_const::<2053930165, 1047731467, 113079296, 343880178>()].span(),
            [qm31_const::<1718384270, 376642827, 1925018623, 343880177>()].span(),
            [qm31_const::<1651275091, 242425099, 1857909759, 343880177>()].span(),
            [qm31_const::<1852602628, 645078283, 2059236351, 343880177>()].span(),
            [qm31_const::<1785493449, 510860555, 1992127487, 343880177>()].span(),
            [qm31_const::<1449947554, 1987255562, 1656583166, 343880177>()].span(),
            [qm31_const::<1382838375, 1853037834, 1589474302, 343880177>()].span(),
            [qm31_const::<510356977, 108207322, 717059022, 343880161>()].span(),
            [qm31_const::<577466156, 242425050, 784167886, 343880161>()].span(),
            [qm31_const::<376138619, 1987255513, 582841293, 343880161>()].span(),
            [qm31_const::<443247798, 2121473241, 649950157, 343880161>()].span(),
            [qm31_const::<778793693, 645078234, 985494478, 343880161>()].span(),
            [qm31_const::<845902872, 779295962, 1052603342, 343880161>()].span(),
            [qm31_const::<644575335, 376642778, 851276750, 343880161>()].span(),
            [qm31_const::<711684514, 510860506, 918385614, 343880161>()].span(),
            [qm31_const::<1047230409, 1181949146, 1253929934, 343880161>()].span(),
            [qm31_const::<1114339588, 1316166874, 1321038798, 343880161>()].span(),
            [qm31_const::<1717810224, 376642479, 1925018275, 343880061>()].span(),
            [qm31_const::<1650701045, 242424751, 1857909411, 343880061>()].span(),
            [qm31_const::<1583591866, 108207023, 1790800547, 343880061>()].span(),
            [qm31_const::<1516482687, 2121472942, 1723691682, 343880061>()].span(),
            [qm31_const::<1986246940, 913513391, 45970084, 343880062>()].span(),
            [qm31_const::<1919137761, 779295663, 2126344867, 343880061>()].span(),
            [qm31_const::<1852028582, 645077935, 2059236003, 343880061>()].span(),
            [qm31_const::<1784919403, 510860207, 1992127139, 343880061>()].span(),
            [qm31_const::<1180936792, 1450384302, 1388147362, 343880061>()].span(),
            [qm31_const::<1113827613, 1316166574, 1321038498, 343880061>()].span(),
            [qm31_const::<241305891, 1718819697, 448623205, 343880041>()].span(),
            [qm31_const::<308415070, 1853037425, 515732069, 343880041>()].span(),
            [qm31_const::<375524249, 1987255153, 582840933, 343880041>()].span(),
            [qm31_const::<442633428, 2121472881, 649949797, 343880041>()].span(),
            [qm31_const::<509742607, 108206962, 717058662, 343880041>()].span(),
            [qm31_const::<576851786, 242424690, 784167526, 343880041>()].span(),
            [qm31_const::<643960965, 376642418, 851276390, 343880041>()].span(),
            [qm31_const::<711070144, 510860146, 918385254, 343880041>()].span(),
            [qm31_const::<778179323, 645077874, 985494118, 343880041>()].span(),
            [qm31_const::<845288502, 779295602, 1052602982, 343880041>()].span(),
            [qm31_const::<375831434, 1987255333, 582841113, 343880101>()].span(),
            [qm31_const::<308722255, 1853037605, 515732249, 343880101>()].span(),
            [qm31_const::<510049792, 108207142, 717058842, 343880101>()].span(),
            [qm31_const::<442940613, 2121473061, 649949977, 343880101>()].span(),
            [qm31_const::<644268150, 376642598, 851276570, 343880101>()].span(),
            [qm31_const::<577158971, 242424870, 784167706, 343880101>()].span(),
            [qm31_const::<778486508, 645078054, 985494298, 343880101>()].span(),
            [qm31_const::<711377329, 510860326, 918385434, 343880101>()].span(),
            [qm31_const::<912704866, 913513510, 1119712026, 343880101>()].span(),
            [qm31_const::<845595687, 779295782, 1052603162, 343880101>()].span(),
            [qm31_const::<1046820829, 1181948906, 1253929694, 343880081>()].span(),
            [qm31_const::<1113930008, 1316166634, 1321038558, 343880081>()].span(),
            [qm31_const::<912602471, 913513450, 1119711966, 343880081>()].span(),
            [qm31_const::<979711650, 1047731178, 1186820830, 343880081>()].span(),
            [qm31_const::<778384113, 645077994, 985494238, 343880081>()].span(),
            [qm31_const::<845493292, 779295722, 1052603102, 343880081>()].span(),
            [qm31_const::<644165755, 376642538, 851276510, 343880081>()].span(),
            [qm31_const::<711274934, 510860266, 918385374, 343880081>()].span(),
            [qm31_const::<1583694261, 108207083, 1790800607, 343880081>()].span(),
            [qm31_const::<1650803440, 242424811, 1857909471, 343880081>()].span(),
            [qm31_const::<108388425, 1450385012, 314406248, 343880298>()].span(),
            [qm31_const::<41279246, 1316167284, 247297384, 343880298>()].span(),
            [qm31_const::<2121653714, 1181949555, 180188520, 343880298>()].span(),
            [qm31_const::<2054544535, 1047731827, 113079656, 343880298>()].span(),
            [qm31_const::<1987435356, 913514099, 45970792, 343880298>()].span(),
            [qm31_const::<1920326177, 779296371, 2126345575, 343880297>()].span(),
            [qm31_const::<1853216998, 645078643, 2059236711, 343880297>()].span(),
            [qm31_const::<1786107819, 510860915, 1992127847, 343880297>()].span(),
            [qm31_const::<1718998640, 376643187, 1925018983, 343880297>()].span(),
            [qm31_const::<1651889461, 242425459, 1857910119, 343880297>()].span(),
            [qm31_const::<779367739, 645078582, 985494826, 343880277>()].span(),
            [qm31_const::<846476918, 779296310, 1052603690, 343880277>()].span(),
            [qm31_const::<913586097, 913514038, 1119712554, 343880277>()].span(),
            [qm31_const::<980695276, 1047731766, 1186821418, 343880277>()].span(),
            [qm31_const::<510931023, 108207670, 717059370, 343880277>()].span(),
            [qm31_const::<578040202, 242425398, 784168234, 343880277>()].span(),
            [qm31_const::<645149381, 376643126, 851277098, 343880277>()].span(),
            [qm31_const::<712258560, 510860854, 918385962, 343880277>()].span(),
            [qm31_const::<1316241171, 1718820406, 1522365738, 343880277>()].span(),
            [qm31_const::<1383350350, 1853038134, 1589474602, 343880277>()].span(),
            [qm31_const::<1340598866, 536394231, 1198633759, 502514173>()].span(),
            [qm31_const::<1407708045, 670611959, 1265742623, 502514173>()].span(),
            [qm31_const::<1474817224, 804829687, 1332851487, 502514173>()].span(),
            [qm31_const::<1541926403, 939047415, 1399960351, 502514173>()].span(),
            [qm31_const::<1072162150, 2147006966, 930198302, 502514173>()].span(),
            [qm31_const::<1139271329, 133741047, 997307167, 502514173>()].span(),
            [qm31_const::<1206380508, 267958775, 1064416031, 502514173>()].span(),
            [qm31_const::<1273489687, 402176503, 1131524895, 502514173>()].span(),
            [qm31_const::<1877472298, 1610136055, 1735504671, 502514173>()].span(),
            [qm31_const::<1944581477, 1744353783, 1802613535, 502514173>()].span(),
            [qm31_const::<669619552, 1341700661, 527545181, 502514194>()].span(),
            [qm31_const::<602510373, 1207482933, 460436317, 502514194>()].span(),
            [qm31_const::<535401194, 1073265205, 393327453, 502514194>()].span(),
            [qm31_const::<468292015, 939047477, 326218589, 502514194>()].span(),
            [qm31_const::<401182836, 804829749, 259109725, 502514194>()].span(),
            [qm31_const::<334073657, 670612021, 192000861, 502514194>()].span(),
            [qm31_const::<266964478, 536394293, 124891997, 502514194>()].span(),
            [qm31_const::<199855299, 402176565, 57783133, 502514194>()].span(),
            [qm31_const::<132746120, 267958837, 2138157916, 502514193>()].span(),
            [qm31_const::<65636941, 133741109, 2071049052, 502514193>()].span(),
            [qm31_const::<2146113804, 2147007087, 2003940247, 502514213>()].span(),
            [qm31_const::<65739336, 133741169, 2071049112, 502514213>()].span(),
            [qm31_const::<2011895446, 1878571631, 1869722519, 502514213>()].span(),
            [qm31_const::<2079004625, 2012789359, 1936831383, 502514213>()].span(),
            [qm31_const::<267066873, 536394353, 124892057, 502514214>()].span(),
            [qm31_const::<334176052, 670612081, 192000921, 502514214>()].span(),
            [qm31_const::<132848515, 267958897, 2138157976, 502514213>()].span(),
            [qm31_const::<199957694, 402176625, 57783193, 502514214>()].span(),
            [qm31_const::<1609240372, 1073265263, 1467069335, 502514213>()].span(),
            [qm31_const::<1676349551, 1207482991, 1534178199, 502514213>()].span(),
            [qm31_const::<1475124409, 804829867, 1332851667, 502514233>()].span(),
            [qm31_const::<1408015230, 670612139, 1265742803, 502514233>()].span(),
            [qm31_const::<1609342767, 1073265323, 1467069395, 502514233>()].span(),
            [qm31_const::<1542233588, 939047595, 1399960531, 502514233>()].span(),
            [qm31_const::<1206687693, 267958955, 1064416211, 502514233>()].span(),
            [qm31_const::<1139578514, 133741227, 997307347, 502514233>()].span(),
            [qm31_const::<1340906051, 536394411, 1198633939, 502514233>()].span(),
            [qm31_const::<1273796872, 402176683, 1131525075, 502514233>()].span(),
            [qm31_const::<2011997841, 1878571691, 1869722579, 502514233>()].span(),
            [qm31_const::<1944888662, 1744353963, 1802613715, 502514233>()].span(),
            [qm31_const::<1877062718, 1610135815, 1735504431, 502514093>()].span(),
            [qm31_const::<1944171897, 1744353543, 1802613295, 502514093>()].span(),
            [qm31_const::<2011281076, 1878571271, 1869722159, 502514093>()].span(),
            [qm31_const::<2078390255, 2012788999, 1936831023, 502514093>()].span(),
            [qm31_const::<2145499434, 2147006727, 2003939887, 502514093>()].span(),
            [qm31_const::<65124966, 133740809, 2071048752, 502514093>()].span(),
            [qm31_const::<132234145, 267958537, 2138157616, 502514093>()].span(),
            [qm31_const::<199343324, 402176265, 57782833, 502514094>()].span(),
            [qm31_const::<1340189286, 536393991, 1198633519, 502514093>()].span(),
            [qm31_const::<1407298465, 670611719, 1265742383, 502514093>()].span(),
            [qm31_const::<1206073323, 267958595, 1064415851, 502514113>()].span(),
            [qm31_const::<1138964144, 133740867, 997306987, 502514113>()].span(),
            [qm31_const::<1071854965, 2147006786, 930198122, 502514113>()].span(),
            [qm31_const::<1004745786, 2012789058, 863089258, 502514113>()].span(),
            [qm31_const::<1474510039, 804829507, 1332851307, 502514113>()].span(),
            [qm31_const::<1407400860, 670611779, 1265742443, 502514113>()].span(),
            [qm31_const::<1340291681, 536394051, 1198633579, 502514113>()].span(),
            [qm31_const::<1273182502, 402176323, 1131524715, 502514113>()].span(),
            [qm31_const::<1742946755, 1341700419, 1601286763, 502514113>()].span(),
            [qm31_const::<1675837576, 1207482691, 1534177899, 502514113>()].span(),
            [qm31_const::<535094009, 1073265025, 393327273, 502514134>()].span(),
            [qm31_const::<602203188, 1207482753, 460436137, 502514134>()].span(),
            [qm31_const::<400875651, 804829569, 259109545, 502514134>()].span(),
            [qm31_const::<467984830, 939047297, 326218409, 502514134>()].span(),
            [qm31_const::<266657293, 536394113, 124891817, 502514134>()].span(),
            [qm31_const::<333766472, 670611841, 192000681, 502514134>()].span(),
            [qm31_const::<132438935, 267958657, 2138157736, 502514133>()].span(),
            [qm31_const::<199548114, 402176385, 57782953, 502514134>()].span(),
            [qm31_const::<2145704224, 2147006847, 2003940007, 502514133>()].span(),
        ]
            .span();
        let interaction_values = array![
            qm31_const::<1005168032, 79980996, 1847888101, 1941984119>(),
            qm31_const::<1072277211, 214198724, 1914996965, 1941984119>(),
            qm31_const::<1139386390, 348416452, 1982105829, 1941984119>(),
            qm31_const::<1206495569, 482634180, 2049214693, 1941984119>(),
            qm31_const::<736731316, 1690593731, 1579452644, 1941984119>(),
            qm31_const::<803840495, 1824811459, 1646561508, 1941984119>(),
            qm31_const::<870949674, 1959029187, 1713670372, 1941984119>(),
            qm31_const::<938058853, 2093246915, 1780779236, 1941984119>(),
            qm31_const::<1542041464, 1153722820, 237275366, 1941984120>(),
        ];
        let mut interaction_columns = make_interaction_trace(
            interaction_values, qm31_const::<1115374022, 1127856551, 489657863, 643630026>(),
        );
        component
            .evaluate_constraints_at_point(
                ref sum,
                ref preprocessed_trace,
                ref trace_columns,
                ref interaction_columns,
                qm31_const::<474642921, 876336632, 1911695779, 974600512>(),
                public_params,
            );
        preprocessed_trace.validate_usage();
        assert_eq!(
            sum, QM31Trait::from_fixed_array(POSEIDON_3_PARTIAL_ROUNDS_CHAIN_SAMPLE_EVAL_RESULT),
        )
    }
}
