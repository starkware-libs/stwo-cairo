...
pub fn verify_cairo(proof: CairoProof) {
    ...
    let log_max_trace_size_plus1 = commitment_scheme.trees[1].columns_by_log_size.len();
    assert!(log_max_trace_size_plus1 == commitment_scheme.trees[2].columns_by_log_size.len())
    // `log_max_trace_size_plus1` includes the blowup factor, so we need to subtract it, the plus
    // one accounts for the maximal constraint degree being 2.
    let log_degree_bound = log_max_trace_size_plus1 - pcs_config.fri_config.log_blowup_factor;
