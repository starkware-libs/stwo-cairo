...
use stwo_verifier_utils::{PublicMemoryEntries, PublicMemoryEntriesTrait};
...
pub impl PublicMemoryImpl of PublicMemoryTrait {
    fn get_entries(
        ...
        for (id, value) in self.program {
            pub_memory_entries.add_memory_entry(address: initial_pc + i, id: *id, value: *value);
            i += 1;
        }
        ...
        for (id, value) in self.output {
            pub_memory_entries.add_memory_entry(address: final_ap + i, id: *id, value: *value);
            i += 1;
        }
