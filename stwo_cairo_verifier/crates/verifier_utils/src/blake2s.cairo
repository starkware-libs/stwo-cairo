...
/// Returns the hash of memory section values. If a digest is given, it's hashed together with the
/// values (before them).
/// Note: this function ignores the ids and therefore assumes that the section is sorted.
pub fn hash_memory_section(mut section: MemorySection, digest: Option<[u32; 8]>) -> Box<[u32; 8]> {
    ...
    let (_, [v0, v1, v2, v3, v4, v5, v6, v7]) = if let Some(head) = section.pop_front() {
        *head
        ...
        if let Some([d0, d1, d2, d3, d4, d5, d6, d7]) = digest {
            byte_count += 32;
