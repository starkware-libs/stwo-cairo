...
//! circuit.
use privacy_consts::{LIFTING_LOG_SIZE, N_OUTPUTS, PREPROCESSED_COLUMN_LOG_SIZES, preprocessed_root};
...
pub fn verify_circuit(proof: CircuitProof) {
    ...
    assert!(lifting_log_size == LIFTING_LOG_SIZE);
    ...
    // Override the aggregate preprocessed log sizes with the hardcoded ones.
    let log_sizes = override_preprocessed_trace_log_sizes(
        column_log_sizes_per_tree(component_log_sizes), preprocessed_column_log_sizes.span(),
    );
    ...
    let circuit_air_log_degree_bound = lifting_log_size - log_blowup_factor;
