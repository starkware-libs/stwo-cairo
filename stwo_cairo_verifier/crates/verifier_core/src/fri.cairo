...
fn decommit_inner_layers(
    ...
    let (mut sparse_eval, column_domain) = first_layer_iter
        .next()
        .expect('must have least one inner layer');
    ...
    for layer in verifier.inner_layers.span() {
        ...
        // If we the first layer commitment has a circle poly with the same degree bound as this
        // layer (before it was folded), then we need to fold it into this layer.
