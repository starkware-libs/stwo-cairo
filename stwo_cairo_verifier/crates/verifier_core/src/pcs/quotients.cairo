...
use crate::utils::{
    ArrayImpl as ArrayUtilImpl, ColumnsIndicesPerTreeByDegreeBound, SpanExTrait, SpanImpl,
    bit_reverse_index,
};
...
impl ColumnSampleBatchImpl of ColumnSampleBatchTrait {
    ...
    fn group_by_point(samples_per_column: Array<@Array<PointSample>>) -> Array<ColumnSampleBatch> {
        ...
        for (column_idx, samples) in samples_per_column.span().into_iter().enumerate() {
            ...
            let Some(tuple_box) = samples.try_into() else {
        ...
        let Some((two_point_sample_idx, _)) = indexed_evaluations_at_prev_point
            .span()
            .first() else {
            let Some((one_point_sample_idx, _)) = indexed_evaluations_at_point.span().first() else {
                return array![];
            };
