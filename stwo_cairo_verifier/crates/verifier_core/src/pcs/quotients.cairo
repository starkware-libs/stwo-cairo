...
///   `lifting_log_size >= max_log_degree_bound + log_blowup_factor`, where
///   `max_log_degree_bound = column_indices_per_tree_by_degree_bound.len() - 1`.
pub fn fri_answers(
    ...
    // `column_indices_per_tree_by_degree_bound[i]` holds the columns of log degree bound `i`, so
    // its length is `max_log_degree_bound + 1`.
    let max_log_degree_bound = column_indices_per_tree_by_degree_bound.len() - 1;
