...
mod tests {
    ...
    use crate::fields::qm31::{PackedUnreducedQM31Trait, QM31, QM31Trait, qm31_const};
    use crate::pcs::quotients::{
        ColumnSampleBatch, ColumnSampleBatchImpl, ComplexConjugateLineCoeffsImpl,
        ComplexConjugateLineCoeffsTrait, PointSample, QuotientConstantsImpl,
        accumulate_row_quotients, fri_answers, fri_answers_for_log_size, get_alpha_mul_a,
        get_alpha_mul_b, get_alpha_mul_c,
    };
    ...
    fn test_complex_conjugate_line_coeffs_impl() {
        ...
        let alpha_mul_a = get_alpha_mul_a(res);
        let alpha_mul_b = get_alpha_mul_b(res);
        let alpha_mul_c = get_alpha_mul_c(res);

        assert!(alpha_mul_a == QM31Trait::from_array([126, 2147483415, 8, 2147483581]));
        assert!(alpha_mul_b == QM31Trait::from_array([20238140, 1378415613, 17263450, 142791233]));
        assert!(
            alpha_mul_c == QM31Trait::from_array([865924731, 72415967, 2011255989, 1549931113]),
        );
    }
