...
pub impl QM31Impl of QM31Trait {
    ...
    fn from_array(arr: [M31; 4]) -> QM31 {
        let [a, b, c, d] = arr;
        core::qm31::QM31Trait::new(a.into(), b.into(), c.into(), d.into())
    }
...
impl QM31PartialOrd of PartialOrd<QM31> {
    ...
    fn lt(lhs: QM31, rhs: QM31) -> bool {
        let [la, lb, lc, ld] = lhs.inner.into();
        let [ra, rb, rc, rd] = rhs.inner.into();
        la < ra
            || (la == ra && lb < rb)
            || (la == ra && lb == rb && lc < rc)
...
pub impl PackedUnreducedQM31Impl of PackedUnreducedQM31Trait {
    #[inline]
    fn mul_m31(self: PackedUnreducedQM31, rhs: UnreducedM31) -> PackedUnreducedQM31 {
        PackedUnreducedQM31 { inner: self.inner * rhs.inner }
    }
...
#[cfg(test)]
mod tests {
    use super::super::m31::{M31InnerT, P_U32 as P, m31};
    ...
    fn qm31(a: u32, b: u32, c: u32, d: u32) -> QM31 {
        let a = M31 { inner: a.into() };
        let b = M31 { inner: b.into() };
        let c = M31 { inner: c.into() };
        let d = M31 { inner: d.into() };
