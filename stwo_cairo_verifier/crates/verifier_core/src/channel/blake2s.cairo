...
pub fn new_channel(digest: Box<[u32; 8]>) -> Blake2sChannel {
...
impl Blake2sChannelDefault of Default<Blake2sChannel> {
    fn default() -> Blake2sChannel {
        Blake2sChannel { digest: BoxImpl::new([0; 8]), channel_time: Default::default() }
    }
}

pub impl Blake2sChannelImpl of ChannelTrait<Blake2sChannel> {
    type Hash = Box<[u32; 8]>;

    const BYTES_PER_HASH: usize = 32;

    fn mix_root(ref self: Blake2sChannel, root: Box<[u32; 8]>) {
        let [d0, d1, d2, d3, d4, d5, d6, d7] = self.digest.unbox();
        let [r0, r1, r2, r3, r4, r5, r6, r7] = root.unbox();
    ...
    fn mix_u64(ref self: Blake2sChannel, nonce: u64) {
        ...
        let res = blake2s_compress(self.digest, 0, BoxImpl::new(msg));
        update_digest(ref self, res);
    }
    ...
    fn draw_random_bytes(ref self: Blake2sChannel) -> Array<u8> {
        let words = draw_random_words(ref self).unbox();
        let mut bytes = array![];
    ...
    fn check_proof_of_work(self: @Blake2sChannel, n_bits: u32) -> bool {
        ...
        let [d0, d1, d2, d3, _, _, _, _] = self.digest.unbox();
        let v = d3.into();
...
fn update_digest(ref channel: Blake2sChannel, new_digest: Box<[u32; 8]>) {
...
fn draw_random_words(ref channel: Blake2sChannel) -> Box<[u32; 8]> {
    let [d0, d1, d2, d3, d4, d5, d6, d7] = channel.digest.unbox();
    ...
    blake2s_finalize(BoxImpl::new(BLAKE2S_256_INITIAL_STATE), 64, msg)
}
...
mod tests {
    ...
    fn test_check_proof_of_work() {
        let channel = new_channel(BoxImpl::new([0b1000, 0, 0, 0, 0, 0, 0, 0]));
