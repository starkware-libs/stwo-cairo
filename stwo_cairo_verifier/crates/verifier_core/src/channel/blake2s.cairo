...
pub impl Blake2sChannelImpl of ChannelTrait {
    ...
    fn verify_pow_nonce(self: @Blake2sChannel, n_bits: u32, nonce: u64) -> bool {
        ...
        //          1 u32      || 8 u32  || 1 u32.
        let msg = BoxImpl::new([POW_PREFIX, d0, d1, d2, d3, d4, d5, d6, d7, n_bits, 0, 0, 0, 0, 0, 0]);
        let [q0, q1, q2, q3, q4, q5, q6, q7]= blake2s_finalize(BoxImpl::new(BLAKE2S_256_INITIAL_STATE), 40, msg).unbox();

        let (q, r) = div_rem(nonce, NZ_U32_SHIFT);
        let nonce_hi = upcast(q);
        let nonce_lo = upcast(r);
        let msg = BoxImpl::new([q0, q1, q2, q3, q4, q5, q6, q7, nonce_lo, nonce_hi, 0, 0, 0, 0, 0, 0]);
        let digest = Blake2sHash { hash: blake2s_finalize(BoxImpl::new(BLAKE2S_256_INITIAL_STATE), 40, msg)};
