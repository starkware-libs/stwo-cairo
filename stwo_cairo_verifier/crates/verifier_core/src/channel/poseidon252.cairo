...
pub impl Poseidon252ChannelImpl of ChannelTrait<Poseidon252Channel> {
    type Hash = felt252;
    ...
    fn draw_random_bytes(ref self: Poseidon252Channel) -> Array<u8> {
        let mut cur: u256 = draw_fet252(ref self).into();
        let mut bytes = array![];
        for _ in 0_usize..Self::BYTES_PER_HASH {
...
fn draw_fet252(ref channel: Poseidon252Channel) -> felt252 {
...
mod tests {
    ...
    fn test_channel_time() {
        ...
        // Poseidon252ChannelImpl::draw_felt(ref channel);
        channel.draw_felt();
        assert_eq!(channel.channel_time.n_challenges, 0);
