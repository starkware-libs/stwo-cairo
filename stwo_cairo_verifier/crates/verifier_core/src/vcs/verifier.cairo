...
> of MerkleVerifierTrait<H> {
    ...
    /// all positions in columns with `log_size < max_log_size` contain the foldings of
    /// positions in columns with `log_size == max_log_size` (in practice, the containment is
    /// actually equality).
