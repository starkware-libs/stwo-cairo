...
mod PoseidonTests {
    ...
    /// Compare with the values generated by test in stwo at
    /// [`crates/stwo/src/core/vcs_lifted/poseidon252_merkle.rs`] at commit
    /// 1f6cc1d6e6236c1e538c1a0607d11fbd52672c93.
    #[test]
    fn test_print_values() {
        let max_len: usize = 280;

        for len in 0..max_len {
            let mut values: Array<M31> = array![];
            for i in 0..len {
                values.append(10_usize.try_into().unwrap() + i.try_into().unwrap());
            }
            let result = MerkleHasher::hash_node(None, values.span());
