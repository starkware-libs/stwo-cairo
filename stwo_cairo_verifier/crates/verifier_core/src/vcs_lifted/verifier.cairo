...
pub trait MerkleVerifierLiftedTrait<impl H: MerkleHasher> {
    ...
    /// # Arguments
    ///
    /// * `query_positions` - Vector of queries position with the respect to the largest layer,
    /// in non-decreasing order (in particular, there may be duplications).
    /// * `queried_values` - An array of spans of queried values. For each query position, there is
    /// a row of values, one for each column, at the index corresponding to the query position.
...
> of MerkleVerifierLiftedTrait<H> {
    ...
    fn verify(
        ...
        for _ in 0..*self.tree_height {
            ...
            // The arrays `prev_layer_queries` and `prev_layer_hashes` advance in lock step. The
            // reason for making two different spans instead of a single span of pairs is to use
            // method `next_if_eq`.
        ...
        // Check that the witness and values have been consumed.
